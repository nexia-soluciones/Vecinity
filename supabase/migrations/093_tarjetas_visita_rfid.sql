...
-- en vecino.rfid_tags, y rfid_reconcile_plan solo enrolaba tipo='vehiculo'.
...
ALTER TYPE vecino.tag_type ADD VALUE IF NOT EXISTS 'visita';

-- ── Backfill: visitas entregadas sin tag ─────────────────────────────────────
INSERT INTO vecino.rfid_tags (colonia_id, house_id, codigo_tag, tipo, status, impresa_at)
SELECT cr.colonia_id, cr.house_id, ci.serial,
       'visita'::vecino.tag_type, 'activo'::vecino.tag_status, cr.delivered_at
FROM vecino.card_requests cr
JOIN vecino.card_inventory ci ON ci.card_request_id = cr.id
WHERE cr.tipo = 'visita'
  AND cr.estado = 'entregada'
  AND NOT EXISTS (SELECT 1 FROM vecino.rfid_tags rt
...
BEGIN
  ...
  SELECT COALESCE(jsonb_agg(to_jsonb(x) - 'prio' ORDER BY x.prio), '[]'::jsonb)
    ...
    LEFT JOIN LATERAL (
      ...
      FROM vecino.card_inventory ci
      JOIN vecino.card_requests cr ON cr.id = ci.card_request_id
...
CREATE OR REPLACE FUNCTION vecino.card_delivery_create_tag()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'vecino', 'public'
...
BEGIN
  ...
  SELECT cr.colonia_id, cr.house_id, NEW.serial,
         'visita'::vecino.tag_type, 'activo'::vecino.tag_status, NEW.delivered_at
  FROM vecino.card_requests cr
...
DROP TRIGGER IF EXISTS trg_card_delivery_create_tag ON vecino.card_deliveries;
CREATE TRIGGER trg_card_delivery_create_tag
  AFTER INSERT ON vecino.card_deliveries
  FOR EACH ROW
  EXECUTE FUNCTION vecino.card_delivery_create_tag();
