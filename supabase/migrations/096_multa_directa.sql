...
COMMENT ON COLUMN vecino.incident_reports.origen IS
...
BEGIN
  ...
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM vecino.fine_categories
  ...
  v_rep := vecino.reportar_incidencia(
...
REVOKE ALL ON FUNCTION vecino.levantar_multa(uuid,uuid,numeric,text,text,text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION vecino.levantar_multa(uuid,uuid,numeric,text,text,text)
