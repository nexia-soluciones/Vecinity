...
CREATE INDEX IF NOT EXISTS idx_tx_ref_rastreo
  ON mifracc.transactions (colonia_id, ref_rastreo)
...
CREATE OR REPLACE FUNCTION mifracc._norm_ref(p text)
RETURNS text
...
BEGIN
  UPDATE mifracc.transactions
     SET comprobante_ocr = p_ocr,
         ref_rastreo = nullif(mifracc._norm_ref(p_ref), '')
   WHERE id = p_id
     AND tipo = 'abono'
     AND estado = 'pendiente'
     AND house_id = mifracc.my_house_id();   -- solo su propia casa
...
CREATE OR REPLACE FUNCTION mifracc.conciliar_auto(
  p_banco_hash text,
...
DECLARE
  ...
  v_refnorm text := mifracc._norm_ref(p_banco_concepto);
  v_id      uuid;
  v_house   uuid;
  v_num     text;
  v_cnt     int;
BEGIN
  ...
  UPDATE mifracc.transactions
   ...
   WHERE id = v_id;
  PERFORM mifracc.resolver_transaccion(v_id, true);  -- aprueba + ajusta saldo/estatus (con FOR UPDATE)

  SELECT numero INTO v_num FROM mifracc.houses WHERE id = v_house;
  RETURN jsonb_build_object('ok', true, 'matched', true, 'abono_id', v_id, 'casa', v_num);
END $$;

GRANT EXECUTE ON FUNCTION mifracc.set_abono_ocr(uuid, jsonb, text) TO authenticated;
GRANT EXECUTE ON FUNCTION mifracc.conciliar_auto(text, text, numeric, date) TO authenticated;
