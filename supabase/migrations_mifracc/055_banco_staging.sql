...
--      en mifracc.bank_movs al momento de subir (staging idempotente por hash).
...
CREATE TABLE IF NOT EXISTS mifracc.bank_uploads (
...
ALTER TABLE mifracc.bank_uploads ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS bank_uploads_admin ON mifracc.bank_uploads;
CREATE POLICY bank_uploads_admin ON mifracc.bank_uploads FOR ALL
...
GRANT SELECT, INSERT, UPDATE, DELETE ON mifracc.bank_uploads TO authenticated, service_role;

-- --- 2. Staging: TODAS las filas del banco ------------------------------------
CREATE TABLE IF NOT EXISTS mifracc.bank_movs (
  id          uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  colonia_id  uuid NOT NULL REFERENCES mifracc.colonias(id) ON DELETE CASCADE,
  upload_id   uuid REFERENCES mifracc.bank_uploads(id) ON DELETE SET NULL,
  fecha       date NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_bank_movs_pend
  ON mifracc.bank_movs (colonia_id, estado, fecha) WHERE estado = 'pendiente';
ALTER TABLE mifracc.bank_movs ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS bank_movs_admin ON mifracc.bank_movs;
CREATE POLICY bank_movs_admin ON mifracc.bank_movs FOR ALL
  USING (colonia_id = mifracc.my_colonia_id() AND mifracc.is_admin())
  WITH CHECK (colonia_id = mifracc.my_colonia_id() AND mifracc.is_admin());
GRANT SELECT, INSERT, UPDATE, DELETE ON mifracc.bank_movs TO authenticated, service_role;
...
CREATE OR REPLACE FUNCTION mifracc.subir_corte_banco(p_archivo text, p_filas jsonb)
...
BEGIN
  ...
  INSERT INTO mifracc.bank_uploads (colonia_id, subido_por, archivo)
  ...
  LOOP
    ...
    v_estado := CASE
      ...
      WHEN r.tipo = 'cargo' AND EXISTS (
        SELECT 1 FROM mifracc.colonia_expenses
         WHERE colonia_id = v_col AND banco_hash = r.hash) THEN 'gasto'
      ELSE 'pendiente' END;

    INSERT INTO mifracc.bank_movs
...
END $$;
GRANT EXECUTE ON FUNCTION mifracc.subir_corte_banco(text, jsonb) TO authenticated;
...
CREATE OR REPLACE FUNCTION mifracc.cobertura_banco()
...
BEGIN
  ...
  SELECT greatest(
    (SELECT max(fecha) FROM mifracc.bank_movs WHERE colonia_id = v_col AND estado <> 'descartado'),
    (SELECT max(fecha_pago) FROM mifracc.colonia_expenses
  ...
  RETURN jsonb_build_object(
    ...
    'pendientes_abonos', (SELECT count(*) FROM mifracc.bank_movs
       WHERE colonia_id = v_col AND estado = 'pendiente' AND tipo = 'abono'),
    'pendientes_cargos', (SELECT count(*) FROM mifracc.bank_movs
       WHERE colonia_id = v_col AND estado = 'pendiente' AND tipo = 'cargo'));
END $$;
GRANT EXECUTE ON FUNCTION mifracc.cobertura_banco() TO authenticated;
...
CREATE OR REPLACE FUNCTION mifracc._marcar_mov_banco_tx()
RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER
SET search_path = mifracc AS $$
BEGIN
  UPDATE mifracc.bank_movs
  ...
  RETURN NEW;
END $$;
DROP TRIGGER IF EXISTS trg_marcar_mov_banco ON mifracc.transactions;
CREATE TRIGGER trg_marcar_mov_banco
  AFTER INSERT OR UPDATE OF banco_hash ON mifracc.transactions
  FOR EACH ROW WHEN (NEW.banco_hash IS NOT NULL)
  EXECUTE FUNCTION mifracc._marcar_mov_banco_tx();

CREATE OR REPLACE FUNCTION mifracc._marcar_mov_banco_exp()
RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER
SET search_path = mifracc AS $$
BEGIN
  UPDATE mifracc.bank_movs
  ...
  RETURN NEW;
END $$;
DROP TRIGGER IF EXISTS trg_marcar_mov_banco_exp ON mifracc.colonia_expenses;
CREATE TRIGGER trg_marcar_mov_banco_exp
  AFTER INSERT OR UPDATE OF banco_hash ON mifracc.colonia_expenses
  FOR EACH ROW WHEN (NEW.banco_hash IS NOT NULL)
  EXECUTE FUNCTION mifracc._marcar_mov_banco_exp();

-- --- 6. descartar_mov_banco: sacar de pendientes lo que nunca se conciliará ---
CREATE OR REPLACE FUNCTION mifracc.descartar_mov_banco(p_banco_hash text, p_nota text DEFAULT NULL)
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER
SET search_path = mifracc, auth AS $$
DECLARE v_col uuid := mifracc.my_colonia_id(); v_rows int;
BEGIN
  IF NOT mifracc.is_admin() THEN RAISE EXCEPTION 'Solo el comité.'; END IF;
  UPDATE mifracc.bank_movs
...
END $$;
GRANT EXECUTE ON FUNCTION mifracc.descartar_mov_banco(text, text) TO authenticated;
...
SET search_path = mifracc, auth AS $$
DECLARE
  v_house     uuid := coalesce(p_house_id, mifracc.my_house_id());
  v_col       uuid;
  v_id        uuid;
  v_dup_casa  text;
  v_dup_fecha text;
BEGIN
  ...
  END IF;
  IF v_house NOT IN (SELECT mifracc.my_finance_house_ids()) THEN
  ...
  SELECT colonia_id INTO v_col FROM mifracc.houses WHERE id = v_house;
...
-- registro) y casa del comprobante original para avisarle al mifracc.
...
BEGIN
  ...
  SELECT colonia_id INTO v_col
   ...
   WHERE id = p_id AND tipo = 'abono' AND estado = 'pendiente'
     AND house_id IN (SELECT mifracc.my_finance_house_ids());
...
-- leído por OCR para compararlo contra lo que capturó el mifracc.
CREATE OR REPLACE FUNCTION mifracc.abonos_pendientes_comite()
RETURNS jsonb LANGUAGE plpgsql STABLE SECURITY DEFINER
SET search_path = mifracc, auth AS $$
DECLARE v_col uuid := mifracc.my_colonia_id(); v_out jsonb;
BEGIN
  IF NOT mifracc.is_admin() THEN RAISE EXCEPTION 'Solo el comité.'; END IF;
    ...
    LEFT JOIN LATERAL (
      SELECT b.banco_hash, b.fecha,
             CASE WHEN t.ref_rastreo IS NOT NULL AND length(t.ref_rastreo) >= 6
                       AND position(t.ref_rastreo IN mifracc._norm_ref(b.concepto)) > 0
                  THEN 'rastreo' ELSE 'monto_fecha' END AS match_por
      FROM mifracc.bank_movs b
      WHERE t.tipo = 'abono'
  ...
  RETURN v_out;
END $$;
GRANT EXECUTE ON FUNCTION mifracc.abonos_pendientes_comite() TO authenticated;
...
CREATE OR REPLACE FUNCTION mifracc.aprobar_abono_banco(p_id uuid, p_banco_hash text)
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER
SET search_path = mifracc, auth AS $$
DECLARE
  v_col   uuid := mifracc.my_colonia_id();
...
BEGIN
  ...
  SELECT * INTO t FROM mifracc.transactions WHERE id = p_id FOR UPDATE;
  ...
  UPDATE mifracc.transactions
   ...
   WHERE id = p_id;
  PERFORM mifracc.resolver_transaccion(p_id, true);  -- aprueba + saldo (el trigger marca bank_movs)
...
END $$;
GRANT EXECUTE ON FUNCTION mifracc.aprobar_abono_banco(uuid, text) TO authenticated;
