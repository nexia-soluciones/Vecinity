...
-- mantenimiento (card_requests.pago_estado) y NUNCA tocaba mifracc.bank_movs. Los
...
ALTER TABLE mifracc.bank_movs DROP CONSTRAINT IF EXISTS bank_movs_estado_check;
ALTER TABLE mifracc.bank_movs ADD CONSTRAINT bank_movs_estado_check
...
ALTER TABLE mifracc.card_requests
  ADD COLUMN IF NOT EXISTS banco_hash text;
CREATE INDEX IF NOT EXISTS idx_card_requests_banco_hash
  ON mifracc.card_requests (colonia_id, banco_hash) WHERE banco_hash IS NOT NULL;
...
CREATE OR REPLACE FUNCTION mifracc._resolver_mov_tarjeta(
  p_col uuid, p_banco_hash text, p_card_id uuid, p_nota text)
RETURNS void LANGUAGE plpgsql SECURITY DEFINER SET search_path = mifracc AS $$
DECLARE b mifracc.bank_movs%ROWTYPE;
BEGIN
  SELECT * INTO b FROM mifracc.bank_movs
...
DROP FUNCTION IF EXISTS mifracc.validar_pago_tarjeta(uuid, boolean, text);
CREATE OR REPLACE FUNCTION mifracc.validar_pago_tarjeta(
...
BEGIN
  ...
  IF p_aprobar AND v_hash IS NOT NULL THEN
    PERFORM mifracc._resolver_mov_tarjeta(v_col, v_hash, p_id,
...
END $$;
GRANT EXECUTE ON FUNCTION mifracc.validar_pago_tarjeta(uuid, boolean, text, text) TO authenticated;
...
CREATE OR REPLACE FUNCTION mifracc.marcar_mov_tarjeta(
...
BEGIN
  ...
  IF p_card_id IS NOT NULL THEN
    SELECT * INTO r FROM mifracc.card_requests WHERE id = p_card_id FOR UPDATE;
    IF NOT FOUND OR r.colonia_id <> v_col THEN
  ...
  PERFORM mifracc._resolver_mov_tarjeta(v_col, p_banco_hash, p_card_id,
...
END $$;
GRANT EXECUTE ON FUNCTION mifracc.marcar_mov_tarjeta(text, uuid, text) TO authenticated;
...
CREATE OR REPLACE FUNCTION mifracc.sugerir_banco_tarjeta(p_card_id uuid)
RETURNS jsonb LANGUAGE plpgsql STABLE SECURITY DEFINER SET search_path = mifracc, public AS $$
DECLARE
  v_col  uuid := mifracc.my_colonia_id();
  r      mifracc.card_requests%ROWTYPE;
  v_casa text;
...
BEGIN
  IF NOT mifracc.is_admin() THEN RAISE EXCEPTION 'Solo el comité.'; END IF;
  SELECT * INTO r FROM mifracc.card_requests WHERE id = p_card_id AND colonia_id = v_col;
  IF NOT FOUND THEN RAISE EXCEPTION 'La solicitud no existe o no es de tu colonia.'; END IF;
  SELECT numero INTO v_casa FROM mifracc.houses WHERE id = r.house_id;
  ...
  FROM (
    ...
    WHERE b.colonia_id = v_col AND b.tipo = 'abono' AND b.estado = 'pendiente'
      ...
      AND NOT (cb.num IS NOT NULL AND cb.num <> v_casa
               AND EXISTS (SELECT 1 FROM mifracc.houses hx
                            WHERE hx.colonia_id = v_col AND hx.numero = cb.num))
  ) q;

  RETURN v_out;
END $$;
GRANT EXECUTE ON FUNCTION mifracc.sugerir_banco_tarjeta(uuid) TO authenticated;
