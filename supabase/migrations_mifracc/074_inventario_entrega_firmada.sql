...
CREATE TABLE IF NOT EXISTS mifracc.card_inventory (
  ...
  print_job_id    uuid REFERENCES mifracc.print_jobs(id),
...
CREATE INDEX IF NOT EXISTS card_inventory_next
  ON mifracc.card_inventory (colonia_id, orden) WHERE estado = 'disponible';

ALTER TABLE mifracc.card_inventory ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS card_inventory_select ON mifracc.card_inventory;
CREATE POLICY card_inventory_select ON mifracc.card_inventory FOR SELECT
  USING (mifracc.is_admin());
GRANT SELECT ON mifracc.card_inventory TO authenticated, service_role;
...
CREATE OR REPLACE FUNCTION mifracc.print_registrar_lote(p_colonia uuid, p_desde bigint, p_hasta bigint)
...
BEGIN
  ...
  SELECT COALESCE(max(orden), 0) INTO v_base FROM mifracc.card_inventory WHERE colonia_id = p_colonia;

  INSERT INTO mifracc.card_inventory (colonia_id, serial, orden)
  SELECT p_colonia, s::text, v_base + row_number() OVER ()
  FROM generate_series(p_desde, p_hasta, v_paso) s;
  GET DIAGNOSTICS v_n = ROW_COUNT;

  SELECT count(*) INTO v_disp FROM mifracc.card_inventory
   WHERE colonia_id = p_colonia AND estado = 'disponible';
  -- El contador simple de la colonia sigue siendo la referencia del stock.
  UPDATE mifracc.colonias SET stock_tarjetas = v_disp WHERE id = p_colonia;
...
CREATE OR REPLACE FUNCTION mifracc.print_marcar_danada(p_colonia uuid, p_serial text DEFAULT NULL)
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER
SET search_path = mifracc, public AS $$
DECLARE
  v_id uuid; v_serial text; v_disp integer;
BEGIN
  SELECT id, serial INTO v_id, v_serial FROM mifracc.card_inventory
   WHERE colonia_id = p_colonia AND estado = 'disponible'
  ...
  UPDATE mifracc.card_inventory SET estado = 'danada' WHERE id = v_id;
  SELECT count(*) INTO v_disp FROM mifracc.card_inventory
   WHERE colonia_id = p_colonia AND estado = 'disponible';
  UPDATE mifracc.colonias SET stock_tarjetas = v_disp WHERE id = p_colonia;
...
-- ══ 3. Entrega firmada ══════════════════════════════════════════════════
CREATE TABLE IF NOT EXISTS mifracc.card_deliveries (
  id              uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  print_job_id    uuid NOT NULL UNIQUE REFERENCES mifracc.print_jobs(id),
  card_request_id uuid REFERENCES mifracc.card_requests(id),
  colonia_id      uuid NOT NULL REFERENCES mifracc.colonias(id),
  ...
  delivered_by    uuid REFERENCES mifracc.profiles(id),
  delivered_at    timestamptz NOT NULL DEFAULT now()  -- fecha sellada por el SERVIDOR
);
ALTER TABLE mifracc.card_deliveries ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS card_deliveries_select ON mifracc.card_deliveries;
CREATE POLICY card_deliveries_select ON mifracc.card_deliveries FOR SELECT
  USING (mifracc.is_admin());
GRANT SELECT ON mifracc.card_deliveries TO authenticated, service_role;
-- Escritura SOLO vía RPC (sin policy de INSERT).

CREATE OR REPLACE FUNCTION mifracc.entregar_tarjeta_firmada(
...
BEGIN
  ...
  SELECT * INTO j FROM mifracc.print_jobs
   WHERE id = p_job AND colonia_id = mifracc.my_colonia_id() FOR UPDATE;
  ...
  VALUES (p_job, j.card_request_id, j.colonia_id, trim(p_firmante), p_firma_b64,
          auth.uid())  -- mifracc.profiles.id = auth.uid() en este schema
...
-- ══ Permisos ════════════════════════════════════════════════════════════
REVOKE EXECUTE ON FUNCTION mifracc.print_registrar_lote(uuid,bigint,bigint) FROM PUBLIC, authenticated, anon;
REVOKE EXECUTE ON FUNCTION mifracc.print_marcar_danada(uuid,text) FROM PUBLIC, authenticated, anon;
GRANT EXECUTE ON FUNCTION mifracc.print_registrar_lote(uuid,bigint,bigint) TO service_role;
GRANT EXECUTE ON FUNCTION mifracc.print_marcar_danada(uuid,text) TO service_role;
GRANT EXECUTE ON FUNCTION mifracc.entregar_tarjeta_firmada(uuid,text,text) TO authenticated, service_role;
