...
CREATE UNIQUE INDEX IF NOT EXISTS profiles_telegram_chat_uniq
  ON mifracc.profiles (telegram_chat_id)
  WHERE telegram_chat_id IS NOT NULL;

-- ------------------------------------------------------------
-- Tokens de enlace
-- ------------------------------------------------------------
CREATE TABLE IF NOT EXISTS mifracc.telegram_link_tokens (
...
COMMENT ON TABLE mifracc.telegram_link_tokens IS
...
CREATE INDEX IF NOT EXISTS telegram_link_tokens_perfil_idx
  ON mifracc.telegram_link_tokens (profile_id, estado);

-- Sin políticas: sólo se tocan por SECURITY DEFINER. Un token legible es un
-- token robable.
ALTER TABLE mifracc.telegram_link_tokens ENABLE ROW LEVEL SECURITY;
REVOKE ALL ON mifracc.telegram_link_tokens FROM anon, authenticated;

-- ------------------------------------------------------------
-- telegram_link_token() — la app pide el token del usuario en sesión
-- ------------------------------------------------------------
CREATE OR REPLACE FUNCTION mifracc.telegram_link_token()
RETURNS text
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'mifracc', 'auth', 'extensions', 'public'
AS $$
DECLARE
  v_id  uuid := auth.uid();
  v_tok text;
BEGIN
  ...
  UPDATE mifracc.telegram_link_tokens
     SET estado = 'reemplazado'
   WHERE profile_id = v_id AND estado = 'vivo';

  v_tok := encode(gen_random_bytes(16), 'hex');   -- 128 bits
  INSERT INTO mifracc.telegram_link_tokens (token, profile_id, expires_at)
  VALUES (v_tok, v_id, now() + interval '15 minutes');

  RETURN v_tok;
END $$;

REVOKE ALL ON FUNCTION mifracc.telegram_link_token() FROM PUBLIC;
GRANT EXECUTE ON FUNCTION mifracc.telegram_link_token() TO authenticated, service_role;
...
-- ------------------------------------------------------------
CREATE OR REPLACE FUNCTION mifracc.telegram_link_consumir(p_token text, p_chat text)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
AS $$
DECLARE
  t             mifracc.telegram_link_tokens%ROWTYPE;
  v_nombre      text;
  v_prev_id     uuid;
  v_prev_nombre text;
BEGIN
  ...
  SELECT * INTO t FROM mifracc.telegram_link_tokens
  ...
  IF t.expires_at <= now() THEN
    UPDATE mifracc.telegram_link_tokens SET estado = 'expirado' WHERE token = p_token;
    RETURN jsonb_build_object('ok', false, 'motivo', 'expirado');
  END IF;

  SELECT nombre INTO v_nombre FROM mifracc.profiles
...
REVOKE ALL ON FUNCTION mifracc.telegram_link_consumir(text,text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION mifracc.telegram_link_consumir(text,text) TO anon, service_role;
