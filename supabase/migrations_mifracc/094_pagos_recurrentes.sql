...
-- dato no es cero.
--
-- NOTA (mifracc): la sección 5 original (siembra de 9 gastos recurrentes REALES
-- de Villa Catania — CFE, JUMAPA, Telmex, Vigilancia, etc., con montos y RFCs
-- reales) se movió a _excluidos/094_datos_reales.sql. mifracc solo debe
-- sembrarse con datos sintéticos/demo. El resto del archivo (tablas, motor de
-- estado, RPCs, cron) no depende de esa siembra.

-- ─────────────────────────────────────────────────────────────
-- 1. Catálogo de pagos recurrentes
-- ─────────────────────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS mifracc.recurring_bills (
...
CREATE TABLE IF NOT EXISTS mifracc.recurring_bill_events (
  id                 uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  bill_id            uuid NOT NULL REFERENCES mifracc.recurring_bills(id) ON DELETE CASCADE,
  ...
  marcado_por        uuid REFERENCES mifracc.profiles(id),
...
CREATE INDEX IF NOT EXISTS idx_recurring_bills_colonia ON mifracc.recurring_bills(colonia_id) WHERE activo;
CREATE INDEX IF NOT EXISTS idx_recurring_events_bill   ON mifracc.recurring_bill_events(bill_id);

ALTER TABLE mifracc.recurring_bills       ENABLE ROW LEVEL SECURITY;
ALTER TABLE mifracc.recurring_bill_events ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS recurring_bills_admin  ON mifracc.recurring_bills;
DROP POLICY IF EXISTS recurring_events_admin ON mifracc.recurring_bill_events;

-- Solo comité/admin: es información de tesorería, no transparencia al mifracc.
CREATE POLICY recurring_bills_admin ON mifracc.recurring_bills
  FOR ALL USING (colonia_id = mifracc.my_colonia_id() AND mifracc.is_admin())
  WITH CHECK (colonia_id = mifracc.my_colonia_id() AND mifracc.is_admin());

CREATE POLICY recurring_events_admin ON mifracc.recurring_bill_events
  FOR ALL USING (EXISTS (SELECT 1 FROM mifracc.recurring_bills b
                          WHERE b.id = bill_id AND b.colonia_id = mifracc.my_colonia_id())
                 AND mifracc.is_admin())
  WITH CHECK (EXISTS (SELECT 1 FROM mifracc.recurring_bills b
                       WHERE b.id = bill_id AND b.colonia_id = mifracc.my_colonia_id())
              AND mifracc.is_admin());

GRANT SELECT, INSERT, UPDATE ON mifracc.recurring_bills, mifracc.recurring_bill_events TO authenticated;

-- ─────────────────────────────────────────────────────────────
-- 2. Motor de estado (interno, sin auth: lo usan el wrapper y el cron)
-- ─────────────────────────────────────────────────────────────
CREATE OR REPLACE FUNCTION mifracc._recurring_estado(p_colonia uuid)
RETURNS TABLE (
...
LANGUAGE plpgsql SECURITY DEFINER SET search_path TO 'mifracc','auth' AS $$
DECLARE
...
BEGIN
  SELECT max(fecha), min(fecha) INTO v_cob, v_ini
    FROM mifracc.bank_movs WHERE colonia_id = p_colonia;

  FOR b IN SELECT * FROM mifracc.recurring_bills
  ...
  LOOP
    ...
    BEGIN
      SELECT max(e.fecha_pago_manual) INTO v_manual
        FROM mifracc.recurring_bill_events e WHERE e.bill_id = b.id;
...
CREATE OR REPLACE FUNCTION mifracc.pagos_recurrentes_estado()
RETURNS TABLE (
  bill_id uuid, nombre text, categoria text, periodicidad text,
  cargos_por_periodo int, cargos_ultimo_ciclo int,
  ultimo_pago date, origen text, monto_ultimo numeric,
...
BEGIN
  ...
  RETURN QUERY SELECT * FROM mifracc._recurring_estado(mifracc.my_colonia_id());
END $$;

-- ─────────────────────────────────────────────────────────────
-- 3. "Ya se pagó, pero fuera del banco" — con motivo obligatorio
-- ─────────────────────────────────────────────────────────────
CREATE OR REPLACE FUNCTION mifracc.recurring_bill_marcar_pagado(
  p_bill uuid, p_fecha date, p_motivo text)
RETURNS jsonb
LANGUAGE plpgsql SECURITY DEFINER SET search_path TO 'mifracc','auth' AS $$
DECLARE v_col uuid := mifracc.my_colonia_id(); v_periodo text; v_hoy date;
BEGIN
  IF NOT mifracc.is_admin() THEN RAISE EXCEPTION 'Solo el comité.'; END IF;
  IF NOT EXISTS (SELECT 1 FROM mifracc.recurring_bills WHERE id = p_bill AND colonia_id = v_col) THEN
  ...
  SELECT to_char(proximo_esperado,'YYYY-MM-DD') INTO v_periodo
    FROM mifracc._recurring_estado(v_col) WHERE bill_id = p_bill;

  INSERT INTO mifracc.recurring_bill_events (bill_id, periodo, fecha_pago_manual, marcado_por, marcado_at, motivo)
...
-- ─────────────────────────────────────────────────────────────
CREATE OR REPLACE FUNCTION mifracc.cron_pagos_recurrentes(p_token text, p_dry boolean DEFAULT false)
RETURNS jsonb
LANGUAGE plpgsql SECURITY DEFINER SET search_path TO 'mifracc','auth' AS $$
DECLARE
  c        record;
...
BEGIN
  ...
  FOR c IN SELECT id, nombre FROM mifracc.colonias LOOP
    v_lineas := ''; v_n := 0; v_stale := NULL;

    FOR r IN SELECT * FROM mifracc._recurring_estado(c.id)
    ...
    LOOP
      ...
      IF NOT p_dry THEN
        INSERT INTO mifracc.recurring_bill_events (bill_id, periodo)
        VALUES (r.bill_id, to_char(r.proximo_esperado,'YYYY-MM-DD'))
        ON CONFLICT (bill_id, periodo) DO NOTHING;
      END IF;

      CONTINUE WHEN EXISTS (
        SELECT 1 FROM mifracc.recurring_bill_events e
      ...
      IF NOT p_dry THEN
        UPDATE mifracc.recurring_bill_events
    ...
    IF NOT p_dry THEN
      FOR v_chat IN SELECT p.id, p.telegram_chat_id FROM mifracc.profiles p
                     WHERE p.colonia_id = c.id AND p.role = 'comite'
                       AND p.telegram_chat_id IS NOT NULL
      LOOP
        PERFORM mifracc.tg_send(v_chat.telegram_chat_id, v_msg);
        INSERT INTO mifracc.notifications (colonia_id, profile_id, tipo, mensaje, canal, estado_envio, enviado_at)
...
GRANT EXECUTE ON FUNCTION mifracc.pagos_recurrentes_estado()                        TO authenticated;
GRANT EXECUTE ON FUNCTION mifracc.recurring_bill_marcar_pagado(uuid, date, text)    TO authenticated;
GRANT EXECUTE ON FUNCTION mifracc.cron_pagos_recurrentes(text, boolean)             TO anon, authenticated;
