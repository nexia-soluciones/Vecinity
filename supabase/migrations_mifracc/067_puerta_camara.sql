...
--   · mifracc.door_commands  — cola de aperturas CON TTL de 30s + bitácora.
...
--   · mifracc.camera_state   — singleton por cámara con el último frame JPEG
...
CREATE TABLE IF NOT EXISTS mifracc.door_commands (
...
CREATE INDEX IF NOT EXISTS idx_door_commands_pending
  ON mifracc.door_commands (requested_at) WHERE result IS NULL;

ALTER TABLE mifracc.door_commands ENABLE ROW LEVEL SECURITY;
-- Bitácora visible para quienes pueden operar la puerta; escrituras solo por RPC.
DROP POLICY IF EXISTS door_commands_select ON mifracc.door_commands;
CREATE POLICY door_commands_select ON mifracc.door_commands
  FOR SELECT USING (
    EXISTS (SELECT 1 FROM mifracc.profiles
            WHERE id = auth.uid() AND role IN ('admin','comite','guardia'))
  );

-- 2. Estado de cámara (singleton por cámara) -------------------------------------
CREATE TABLE IF NOT EXISTS mifracc.camera_state (
  camera      text PRIMARY KEY CHECK (camera IN ('peatonal')),
  watch_until timestamptz,
  frame_b64   text,
  frame_at    timestamptz
);
INSERT INTO mifracc.camera_state (camera) VALUES ('peatonal')
ON CONFLICT (camera) DO NOTHING;

ALTER TABLE mifracc.camera_state ENABLE ROW LEVEL SECURITY;
...
BEGIN
  ...
  SELECT count(*) INTO v_pendiente FROM mifracc.door_commands
  ...
  END IF;
  INSERT INTO mifracc.door_commands (door, requested_by)
...
CREATE OR REPLACE FUNCTION mifracc.door_status(p_id uuid)
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path = mifracc, public AS $$
DECLARE r record;
BEGIN
  IF NOT mifracc.is_door_operator() THEN
  ...
  END IF;
  SELECT * INTO r FROM mifracc.door_commands WHERE id = p_id;
...
CREATE OR REPLACE FUNCTION mifracc.camera_view(p_camera text DEFAULT 'peatonal')
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path = mifracc, public AS $$
DECLARE r record;
BEGIN
  IF NOT mifracc.is_door_operator() THEN
...
CREATE OR REPLACE FUNCTION mifracc.bridge_fast_poll(p_token text)
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path = mifracc, public AS $$
...
BEGIN
  ...
  SELECT EXISTS (SELECT 1 FROM mifracc.camera_state
...
CREATE OR REPLACE FUNCTION mifracc.door_mark(
...
CREATE OR REPLACE FUNCTION mifracc.camera_push(
...
-- 10. Grants ------------------------------------------------------------------------
GRANT EXECUTE ON FUNCTION mifracc.is_door_operator() TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION mifracc.door_open(text) TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION mifracc.door_status(uuid) TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION mifracc.camera_view(text) TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION mifracc.bridge_fast_poll(text) TO anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION mifracc.door_mark(text, uuid, boolean, text) TO anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION mifracc.camera_push(text, text, text) TO anon, authenticated, service_role;
