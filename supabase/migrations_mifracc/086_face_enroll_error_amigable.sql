...
BEGIN
  ...
  ELSIF p_action = 'error' THEN
    ...
    IF v_rows > 0 THEN
      SELECT numero INTO v_casa FROM mifracc.houses WHERE id = r_face.house_id;
      FOR rec IN
        SELECT telegram_chat_id FROM mifracc.profiles
         WHERE house_id = r_face.house_id AND telegram_chat_id IS NOT NULL
        UNION
        SELECT p.telegram_chat_id FROM mifracc.house_members hm
          JOIN mifracc.profiles p ON p.id = hm.profile_id
         WHERE hm.house_id = r_face.house_id AND p.telegram_chat_id IS NOT NULL
      LOOP
        PERFORM mifracc.tg_send(rec.telegram_chat_id,
