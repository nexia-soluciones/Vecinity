...
ALTER TABLE mifracc.incident_reports ALTER COLUMN infractor_house_id DROP NOT NULL;

-- --- RPC: reportar sin casa (solo evidencia) --------------------------------
-- Requiere evidencia (foto): sin casa Y sin foto no hay nada que revisar.
CREATE OR REPLACE FUNCTION mifracc.reportar_incidencia_sin_casa(
  p_categoria    uuid,
...
BEGIN
  ...
  VALUES
    (v_col, mifracc.my_house_id(), NULL, p_categoria, nullif(btrim(p_descripcion),''),
     p_evidencia_url, now(), p_lat, p_lng, 'sin_identificar')
  RETURNING id INTO v_id;

  RETURN jsonb_build_object('ok', true, 'id', v_id);
END $$;
GRANT EXECUTE ON FUNCTION mifracc.reportar_incidencia_sin_casa(uuid,text,text,numeric,numeric) TO authenticated, service_role;
...
CREATE OR REPLACE FUNCTION mifracc.corregir_casa_infractora(p_id uuid, p_numero text)
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER
SET search_path = mifracc, auth AS $$
DECLARE
...
BEGIN
  ...
  SELECT estado INTO v_estado FROM mifracc.incident_reports
  ...
  SELECT id INTO v_house FROM mifracc.houses
  ...
  UPDATE mifracc.incident_reports
     SET infractor_house_id = v_house,
         estado = CASE WHEN estado = 'sin_identificar' THEN 'pendiente'::mifracc.incident_status ELSE estado END
   WHERE id = p_id;
  RETURN jsonb_build_object('ok', true, 'numero', btrim(p_numero));
END $$;
GRANT EXECUTE ON FUNCTION mifracc.corregir_casa_infractora(uuid,text) TO authenticated, service_role;
