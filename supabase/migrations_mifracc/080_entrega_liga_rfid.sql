...
DROP FUNCTION IF EXISTS mifracc.entregar_tarjeta_firmada(uuid, text, text, text);
...
BEGIN
  ...
  SELECT COALESCE(
    (SELECT serial FROM mifracc.card_inventory WHERE print_job_id = p_job LIMIT 1),
...
GRANT EXECUTE ON FUNCTION mifracc.entregar_tarjeta_firmada(uuid, text, text, text, text) TO authenticated, service_role;
