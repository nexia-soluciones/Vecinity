...
CREATE OR REPLACE FUNCTION mifracc.is_door_operator(p_device text DEFAULT 'peatonal')
RETURNS boolean LANGUAGE sql SECURITY DEFINER STABLE
SET search_path = mifracc, auth AS $$
  SELECT EXISTS (
    SELECT 1 FROM mifracc.profiles p
    JOIN mifracc.camera_state cs ON cs.camera = p_device
    WHERE p.id = auth.uid()
      AND p.approval_status = 'aprobado'
      AND p.colonia_id = cs.colonia_id)
$$;
GRANT EXECUTE ON FUNCTION mifracc.is_door_operator(text) TO authenticated, service_role;
...
BEGIN
  IF NOT mifracc.is_door_operator(p_door) THEN
...
DECLARE r record;
BEGIN
  IF NOT mifracc.is_door_operator(p_camera) THEN
...
BEGIN
  ...
  IF NOT mifracc.is_door_operator(r.door) THEN
...
BEGIN
  IF NOT EXISTS (
      ...
      WHERE p.id = auth.uid()
        ...
        AND p.approval_status = 'aprobado'
        AND p.colonia_id IN (SELECT colonia_id FROM mifracc.camera_state)) THEN
...
CREATE POLICY door_commands_select ON mifracc.door_commands
  FOR SELECT USING (
    EXISTS (SELECT 1 FROM mifracc.profiles p
            WHERE p.id = auth.uid()
              AND p.role IN ('admin','comite','guardia')
              AND p.colonia_id IN (SELECT colonia_id FROM mifracc.camera_state))
  );
