...
CREATE OR REPLACE FUNCTION mifracc.registrar_abono_admin(
  p_house_id        uuid,
  p_monto           numeric,
  p_concepto        text,
...
BEGIN
  ...
  PERFORM mifracc.resolver_transaccion(v_id, true);  -- aprueba + aplica saldo/estatus
  RETURN jsonb_build_object('ok', true, 'id', v_id);
END $$;

GRANT EXECUTE ON FUNCTION mifracc.registrar_abono_admin(uuid,numeric,text,text,text,jsonb,text,date)
