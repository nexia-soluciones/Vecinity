...
-- 1. Tabla ---------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS mifracc.face_enrollments (
...
CREATE INDEX IF NOT EXISTS idx_face_enrollments_house ON mifracc.face_enrollments(house_id);
CREATE INDEX IF NOT EXISTS idx_face_enrollments_status ON mifracc.face_enrollments(status);

ALTER TABLE mifracc.face_enrollments ENABLE ROW LEVEL SECURITY;
-- Solo SELECT directo (listas de la casa / comité). Escrituras: solo RPCs.
DROP POLICY IF EXISTS face_enrollments_select ON mifracc.face_enrollments;
CREATE POLICY face_enrollments_select ON mifracc.face_enrollments
  FOR SELECT USING (
    mifracc.is_admin()
    OR house_id IN (SELECT house_id FROM mifracc.profiles WHERE id = auth.uid())
    OR house_id IN (SELECT house_id FROM mifracc.house_members WHERE profile_id = auth.uid())
  );

-- 2. RPC: el vecino manda una foto ----------------------------------------------
CREATE OR REPLACE FUNCTION mifracc.face_submit(
...
BEGIN
  ...
  END IF;
  SELECT count(*) INTO v_activos FROM mifracc.face_enrollments
  ...
  END IF;
  INSERT INTO mifracc.face_enrollments (house_id, profile_id, nombre, photo_b64)
...
CREATE OR REPLACE FUNCTION mifracc.face_retire(p_id uuid)
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path = mifracc, public AS $$
DECLARE r record;
BEGIN
  SELECT * INTO r FROM mifracc.face_enrollments WHERE id = p_id FOR UPDATE;
  ...
  IF NOT mifracc.is_admin() THEN
    IF r.house_id NOT IN (
        ...
        UNION SELECT house_id FROM mifracc.house_members WHERE profile_id = auth.uid()) THEN
    ...
    END IF;
  END IF;
  UPDATE mifracc.face_enrollments SET status = 'retirada' WHERE id = p_id;
  RETURN jsonb_build_object('ok', true,
    'pendiente_borrado', r.enrolled_at IS NOT NULL AND r.terminal_removed_at IS NULL);
END $$;

-- 4. RPC: el comité aprueba o rechaza -------------------------------------------
CREATE OR REPLACE FUNCTION mifracc.face_review(p_id uuid, p_aprobar boolean, p_motivo text DEFAULT NULL)
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path = mifracc, public AS $$
DECLARE r record;
BEGIN
  IF NOT mifracc.is_admin() THEN
  ...
  END IF;
  IF p_aprobar THEN
    UPDATE mifracc.face_enrollments
...
CREATE OR REPLACE FUNCTION mifracc.face_photo(p_id uuid)
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path = mifracc, public AS $$
DECLARE r record;
BEGIN
  SELECT house_id, photo_b64 INTO r FROM mifracc.face_enrollments WHERE id = p_id;
  IF NOT FOUND THEN RAISE EXCEPTION 'Registro no encontrado.'; END IF;
  IF NOT mifracc.is_admin() AND r.house_id NOT IN (
      SELECT house_id FROM mifracc.profiles WHERE id = auth.uid()
      UNION SELECT house_id FROM mifracc.house_members WHERE profile_id = auth.uid()) THEN
    RAISE EXCEPTION 'Ese rostro no es de tu casa.';
  END IF;
...
CREATE OR REPLACE FUNCTION mifracc.face_sync_plan(p_token text)
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path = mifracc, public AS $$
...
BEGIN
  ...
  SELECT COALESCE(jsonb_agg(to_jsonb(x)), '[]'::jsonb) INTO v_plan FROM (
    ...
    FROM mifracc.face_enrollments f
    JOIN mifracc.houses h ON h.id = f.house_id
...
CREATE OR REPLACE FUNCTION mifracc.face_mark(
...
CREATE OR REPLACE FUNCTION mifracc.face_panel_data()
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path = mifracc, public AS $$
BEGIN
  IF NOT mifracc.is_admin() THEN
  ...
  RETURN jsonb_build_object(
    'pendientes', (
      SELECT COALESCE(jsonb_agg(to_jsonb(p) ORDER BY p.created_at), '[]'::jsonb) FROM (
        ...
        FROM mifracc.face_enrollments f
        JOIN mifracc.houses h ON h.id = f.house_id
        LEFT JOIN mifracc.profiles pr ON pr.id = f.profile_id
    ...
    'pendiente_borrado', (
      SELECT count(*) FROM mifracc.face_enrollments
...
-- 9. GRANTs -----------------------------------------------------------------------
GRANT EXECUTE ON FUNCTION mifracc.face_submit(text, text, uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION mifracc.face_retire(uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION mifracc.face_review(uuid, boolean, text) TO authenticated;
GRANT EXECUTE ON FUNCTION mifracc.face_photo(uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION mifracc.face_panel_data() TO authenticated;
-- Las de la Orin viajan con anon key + token de bot (mismo patrón que rfid_*)
GRANT EXECUTE ON FUNCTION mifracc.face_sync_plan(text) TO anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION mifracc.face_mark(text, uuid, text, text) TO anon, authenticated, service_role;
