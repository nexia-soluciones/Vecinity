...
CREATE OR REPLACE FUNCTION mifracc.solicitar_aclaracion_abono(
  p_id      uuid,
...
END $$;
GRANT EXECUTE ON FUNCTION mifracc.solicitar_aclaracion_abono(uuid, text) TO authenticated;

-- --- El vecino responde ---------------------------------------------------------
-- La fecha respondida se inyecta al comprobante_ocr → el cruce con el banco la usa.
CREATE OR REPLACE FUNCTION mifracc.responder_aclaracion_abono(
  p_id    uuid,
...
END $$;
GRANT EXECUTE ON FUNCTION mifracc.responder_aclaracion_abono(uuid, date, text) TO authenticated;
