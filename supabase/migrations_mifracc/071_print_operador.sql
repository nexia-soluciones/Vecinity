...
CREATE OR REPLACE FUNCTION mifracc.print_take_selected(p_ids uuid[])
...
CREATE OR REPLACE FUNCTION mifracc.print_set_stock(p_colonia uuid, p_stock integer)
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER
SET search_path = mifracc, public AS $$
BEGIN
  ...
  END IF;
  UPDATE mifracc.colonias SET stock_tarjetas = p_stock WHERE id = p_colonia;
...
REVOKE EXECUTE ON FUNCTION mifracc.print_take_selected(uuid[]) FROM PUBLIC, authenticated, anon;
REVOKE EXECUTE ON FUNCTION mifracc.print_reprint(uuid) FROM PUBLIC, authenticated, anon;
REVOKE EXECUTE ON FUNCTION mifracc.print_set_stock(uuid, integer) FROM PUBLIC, authenticated, anon;
GRANT EXECUTE ON FUNCTION mifracc.print_take_selected(uuid[]) TO service_role;
GRANT EXECUTE ON FUNCTION mifracc.print_reprint(uuid) TO service_role;
GRANT EXECUTE ON FUNCTION mifracc.print_set_stock(uuid, integer) TO service_role;
