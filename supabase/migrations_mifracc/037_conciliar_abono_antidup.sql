...
DROP FUNCTION IF EXISTS mifracc.conciliar_abono(uuid, numeric, text, text, text);
...
DECLARE
  ...
  v_id           uuid;
  v_match_id     uuid;
  v_match_estado mifracc.approval_state;
BEGIN
  ...
  IF v_match_id IS NOT NULL THEN
    ...
    IF v_match_estado = 'pendiente' THEN
      PERFORM mifracc.resolver_transaccion(v_match_id, true);  -- aprueba + saldo (una vez)
    END IF;
...
GRANT EXECUTE ON FUNCTION mifracc.conciliar_abono(uuid,numeric,text,text,text,date)
