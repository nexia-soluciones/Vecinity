...
-- ------------------------------------------------------------
DROP POLICY IF EXISTS incident_reports_read ON mifracc.incident_reports;
CREATE POLICY incident_reports_read ON mifracc.incident_reports FOR SELECT
  USING (
    (colonia_id = mifracc.my_colonia_id() AND mifracc.is_admin())
    OR reportante_house_id = mifracc.my_house_id()
  );
...
-- estado de cuenta.
-- ------------------------------------------------------------
CREATE OR REPLACE FUNCTION mifracc.ver_resolucion_multa(p_transaction_id uuid)
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER STABLE
SET search_path = mifracc, auth AS $$
DECLARE
  r   mifracc.incident_reports%ROWTYPE;
  a   mifracc.reglamento%ROWTYPE;
  v_cat text;
BEGIN
  SELECT * INTO r FROM mifracc.incident_reports WHERE transaction_id = p_transaction_id;
  ...
  IF NOT ( r.infractor_house_id = mifracc.my_house_id()
           OR (mifracc.is_admin() AND r.colonia_id = mifracc.my_colonia_id()) ) THEN
  ...
  SELECT rg.* INTO a
    ...
    LEFT JOIN mifracc.reglamento rg ON rg.id = fc.articulo_id
...
-- ------------------------------------------------------------
CREATE OR REPLACE FUNCTION mifracc.set_resolucion_oficial(
  p_incident_id uuid,
  p_texto       text,
...
-- ------------------------------------------------------------
ALTER TABLE mifracc.colonias ALTER COLUMN tope_multa DROP NOT NULL;
ALTER TABLE mifracc.colonias ALTER COLUMN tope_multa DROP DEFAULT;
...
UPDATE mifracc.colonias SET tope_multa = NULL WHERE id = 'ce43b59c-529b-4960-8dd7-d975e43ac2fb';
...
BEGIN
  ...
  END IF;
  SELECT * INTO r FROM mifracc.incident_reports WHERE id = p_id FOR UPDATE;
...
GRANT EXECUTE ON FUNCTION mifracc.ver_resolucion_multa(uuid)            TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION mifracc.set_resolucion_oficial(uuid,text,text) TO service_role;
