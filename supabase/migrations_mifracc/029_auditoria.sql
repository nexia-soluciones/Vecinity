...
CREATE OR REPLACE FUNCTION mifracc.auditoria_abonos_duplicados()
...
BEGIN
  ...
  RETURN QUERY
    ...
    WHERE t.tipo = 'abono' AND t.estado = 'aprobado'
      AND h.colonia_id = mifracc.my_colonia_id()
...
CREATE OR REPLACE FUNCTION mifracc.corregir_abono_duplicado(p_id uuid)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'mifracc', 'auth'
AS $$
DECLARE t mifracc.transactions%ROWTYPE;
BEGIN
  IF NOT mifracc.is_admin() THEN RAISE EXCEPTION 'Solo el comité.'; END IF;
  ...
  IF t.colonia_id <> mifracc.my_colonia_id() THEN RAISE EXCEPTION 'No es de tu colonia.'; END IF;
  IF t.tipo <> 'abono' THEN RAISE EXCEPTION 'Solo se pueden corregir abonos.'; END IF;

  IF t.estado = 'aprobado' THEN
    UPDATE mifracc.houses SET saldo = saldo + t.monto WHERE id = t.house_id;
  ...
  DELETE FROM mifracc.transactions WHERE id = p_id;
  RETURN jsonb_build_object('ok', true);
END $$;

GRANT EXECUTE ON FUNCTION mifracc.auditoria_abonos_duplicados() TO authenticated;
GRANT EXECUTE ON FUNCTION mifracc.corregir_abono_duplicado(uuid) TO authenticated;
