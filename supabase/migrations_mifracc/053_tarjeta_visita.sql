...
ALTER TABLE mifracc.card_requests DROP CONSTRAINT card_req_destino;
ALTER TABLE mifracc.card_requests ADD CONSTRAINT card_req_destino CHECK (
...
CREATE UNIQUE INDEX card_req_unico_visita
  ON mifracc.card_requests (house_id, lower(btrim(beneficiario_nombre)))
...
DROP FUNCTION mifracc.solicitar_tarjeta(mifracc.card_type, uuid, uuid, text);

CREATE FUNCTION mifracc.solicitar_tarjeta(
...
CREATE OR REPLACE FUNCTION mifracc.verificar_tarjeta_visita(p_card_id uuid)
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER
SET search_path = mifracc, public AS $$
DECLARE
  v_rol text;
...
BEGIN
  ...
  SELECT * INTO r FROM mifracc.card_requests
   WHERE id = p_card_id AND tipo = 'visita' AND colonia_id = mifracc.my_colonia_id();
  IF NOT FOUND THEN
...
GRANT EXECUTE ON FUNCTION mifracc.solicitar_tarjeta(mifracc.card_type,uuid,uuid,text,text) TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION mifracc.verificar_tarjeta_visita(uuid) TO authenticated, service_role;
