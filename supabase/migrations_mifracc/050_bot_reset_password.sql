...
CREATE OR REPLACE FUNCTION mifracc.bot_email(p_token text, p_chat text)
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER
SET search_path = mifracc, auth AS $$
DECLARE p mifracc.profiles%ROWTYPE; v_email text;
BEGIN
  p := mifracc._bot_perfil(p_token, p_chat);            -- valida token + liga
...
GRANT EXECUTE ON FUNCTION mifracc.bot_email(text, text) TO anon, authenticated, service_role;
