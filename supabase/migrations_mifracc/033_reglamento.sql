...
CREATE TABLE IF NOT EXISTS mifracc.reglamento (
...
CREATE UNIQUE INDEX IF NOT EXISTS uq_reglamento_colonia_num
  ON mifracc.reglamento (colonia_id, num);
CREATE INDEX IF NOT EXISTS idx_reglamento_colonia ON mifracc.reglamento (colonia_id);

-- Cada categoría de multa apunta al artículo que sanciona (nullable: no toda
-- categoría tiene un artículo exacto).
ALTER TABLE mifracc.fine_categories
  ADD COLUMN IF NOT EXISTS articulo_id uuid REFERENCES mifracc.reglamento(id) ON DELETE SET NULL;

-- ------------------------------------------------------------
-- RLS: el reglamento es público para los vecinos de la colonia (lo consultan);
-- solo el comité lo edita.
-- ------------------------------------------------------------
ALTER TABLE mifracc.reglamento ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS reglamento_read  ON mifracc.reglamento;
DROP POLICY IF EXISTS reglamento_admin ON mifracc.reglamento;

CREATE POLICY reglamento_read ON mifracc.reglamento FOR SELECT
  USING (colonia_id = mifracc.my_colonia_id());
CREATE POLICY reglamento_admin ON mifracc.reglamento FOR ALL
...
GRANT SELECT ON mifracc.reglamento TO authenticated;
GRANT ALL    ON mifracc.reglamento TO service_role;
