...
-- ------------------------------------------------------------
CREATE TABLE IF NOT EXISTS mifracc.soporte_acceso_log (
  id                uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  colonia_id        uuid NOT NULL REFERENCES mifracc.colonias(id) ON DELETE CASCADE,
  actor_id          uuid REFERENCES mifracc.profiles(id) ON DELETE SET NULL,
  actor_nombre      text,
  target_profile_id uuid REFERENCES mifracc.profiles(id) ON DELETE SET NULL,
  target_nombre     text,
  target_email      text,
...
CREATE INDEX IF NOT EXISTS idx_soporte_acceso_log_colonia
  ON mifracc.soporte_acceso_log(colonia_id, created_at DESC);

ALTER TABLE mifracc.soporte_acceso_log ENABLE ROW LEVEL SECURITY;
...
DROP POLICY IF EXISTS soporte_acceso_log_admin_read ON mifracc.soporte_acceso_log;
CREATE POLICY soporte_acceso_log_admin_read ON mifracc.soporte_acceso_log
  FOR SELECT USING (colonia_id = mifracc.my_colonia_id() AND mifracc.is_admin());

GRANT SELECT ON mifracc.soporte_acceso_log TO authenticated;
GRANT ALL    ON mifracc.soporte_acceso_log TO service_role;
...
-- ------------------------------------------------------------
CREATE OR REPLACE FUNCTION mifracc.soporte_buscar_cuentas(p_texto text)
RETURNS jsonb
       ...
       AND (
         ...
         OR EXISTS (
           SELECT 1
            ...
            WHERE (
                    p.house_id = h.id
                    OR EXISTS (SELECT 1 FROM mifracc.house_members hm
...
GRANT EXECUTE ON FUNCTION mifracc.soporte_buscar_cuentas(text) TO authenticated, service_role;

-- ------------------------------------------------------------
-- 3) Reactivar (o crear) el código de invitación CAT-<casa>
--    Último recurso: casa sin cuenta, o cuenta irrecuperable.
-- ------------------------------------------------------------
CREATE OR REPLACE FUNCTION mifracc.soporte_reactivar_invitacion(p_house_id uuid)
RETURNS jsonb
LANGUAGE plpgsql SECURITY DEFINER
SET search_path = mifracc, auth AS $$
DECLARE
  h           mifracc.houses%ROWTYPE;
  v_inv       mifracc.invitations%ROWTYPE;
  v_token     text;
  v_base      text;
  v_k         int := 1;
...
BEGIN
  ...
  SELECT * INTO h FROM mifracc.houses
   WHERE id = p_house_id AND colonia_id = mifracc.my_colonia_id();
  IF NOT FOUND THEN
  ...
  SELECT * INTO v_inv FROM mifracc.invitations
   WHERE house_id = h.id AND relacion IS NULL
   ORDER BY created_at DESC LIMIT 1;

  IF FOUND THEN
    UPDATE mifracc.invitations
  ...
  ELSE
    ...
    v_token := v_base;
    WHILE EXISTS (SELECT 1 FROM mifracc.invitations WHERE token = v_token) LOOP
      v_k := v_k + 1;
      v_token := v_base || '-' || v_k;
    END LOOP;
    INSERT INTO mifracc.invitations (colonia_id, house_id, role, token, invited_by, expires_at)
    VALUES (h.colonia_id, h.id, 'residente', v_token, auth.uid(), v_vence);
  END IF;

  INSERT INTO mifracc.soporte_acceso_log
  ...
  SELECT h.colonia_id, auth.uid(), p.nombre, h.id, h.numero, 'reactivar_invitacion',
    ...
    FROM mifracc.profiles p WHERE p.id = auth.uid();
...
GRANT EXECUTE ON FUNCTION mifracc.soporte_reactivar_invitacion(uuid) TO authenticated, service_role;
...
-- ------------------------------------------------------------
CREATE OR REPLACE FUNCTION mifracc.soporte_baja_cuenta(p_profile_id uuid, p_motivo text DEFAULT NULL)
RETURNS jsonb
LANGUAGE plpgsql SECURITY DEFINER
SET search_path = mifracc, auth AS $$
DECLARE
  t        mifracc.profiles%ROWTYPE;
  v_casa   text;
  v_house  uuid;
  v_tenia_tg boolean;
BEGIN
  ...
  SELECT * INTO t FROM mifracc.profiles
  ...
  v_house := coalesce(t.house_id,
    (SELECT hm.house_id FROM mifracc.house_members hm WHERE hm.profile_id = t.id LIMIT 1));
  SELECT numero INTO v_casa FROM mifracc.houses WHERE id = v_house;
...
GRANT EXECUTE ON FUNCTION mifracc.soporte_baja_cuenta(uuid, text) TO authenticated, service_role;

-- ------------------------------------------------------------
-- 5) Deshacer la baja (si fue un error)
-- ------------------------------------------------------------
CREATE OR REPLACE FUNCTION mifracc.soporte_reactivar_cuenta(p_profile_id uuid)
RETURNS jsonb
LANGUAGE plpgsql SECURITY DEFINER
SET search_path = mifracc, auth AS $$
DECLARE t mifracc.profiles%ROWTYPE;
BEGIN
  IF NOT mifracc.is_admin() THEN
  ...
  INSERT INTO mifracc.soporte_acceso_log
    (colonia_id, actor_id, actor_nombre, target_profile_id, target_nombre, target_email,
     house_id, casa, accion, detalle)
  SELECT t.colonia_id, auth.uid(), p.nombre, t.id, t.nombre, t.email,
         t.house_id, (SELECT numero FROM mifracc.houses WHERE id = t.house_id),
...
GRANT EXECUTE ON FUNCTION mifracc.soporte_reactivar_cuenta(uuid) TO authenticated, service_role;
