...
CREATE OR REPLACE FUNCTION mifracc.calendario_reservas(p_desde date, p_hasta date)
 RETURNS TABLE(
   reserva_id uuid,
   area_id uuid,
   area_nombre text,
...
AS $function$
  SELECT
    ...
    h.numero::text,
    (r.house_id = mifracc.my_house_id()) AS es_mia,
  ...
  WHERE a.colonia_id = mifracc.my_colonia_id()
...
GRANT EXECUTE ON FUNCTION mifracc.calendario_reservas(date, date) TO authenticated;
