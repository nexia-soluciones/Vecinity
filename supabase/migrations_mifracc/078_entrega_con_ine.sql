...
-- En la sesión de entrega (tablet del comité): foto del INE + firma del mifracc.
...
ALTER TABLE mifracc.card_deliveries
  ADD COLUMN IF NOT EXISTS ine_path text;  -- ruta en el bucket privado vecino-ine

-- Bucket privado para las fotos de INE.
INSERT INTO storage.buckets (id, name, public)
VALUES ('mifracc-ine', 'mifracc-ine', false)
ON CONFLICT (id) DO NOTHING;

DROP POLICY IF EXISTS mifracc_ine_insert ON storage.objects;
CREATE POLICY mifracc_ine_insert ON storage.objects FOR INSERT TO authenticated
  WITH CHECK (bucket_id = 'mifracc-ine');
DROP POLICY IF EXISTS mifracc_ine_select ON storage.objects;
CREATE POLICY mifracc_ine_select ON storage.objects FOR SELECT TO authenticated
  USING (bucket_id = 'mifracc-ine');

-- Param nuevo con DEFAULT = sobrecarga ambigua en PostgREST → DROP de la firma vieja.
DROP FUNCTION IF EXISTS mifracc.entregar_tarjeta_firmada(uuid, text, text);
...
GRANT EXECUTE ON FUNCTION mifracc.entregar_tarjeta_firmada(uuid, text, text, text) TO authenticated, service_role;
