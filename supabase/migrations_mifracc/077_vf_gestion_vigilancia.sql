...
CREATE OR REPLACE FUNCTION mifracc.vf_listar()
...
BEGIN
  ...
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
      ...
      'adentro', EXISTS (SELECT 1 FROM mifracc.visitors vi
                          WHERE vi.card_request_id = r.id AND vi.estado = 'adentro')
    ) ORDER BY r.beneficiario_nombre), '[]'::jsonb)
    INTO v
  FROM mifracc.card_requests r
  LEFT JOIN mifracc.houses h ON h.id = r.house_id
  WHERE r.tipo = 'visita'
    AND r.colonia_id = mifracc.my_colonia_id()
    AND r.estado IN ('solicitada','en_cola','impresa','entregada');
  RETURN v;
END $$;

-- Corregir el nombre del titular de la tarjeta.
CREATE OR REPLACE FUNCTION mifracc.vf_editar(p_card_id uuid, p_nombre text)
...
CREATE OR REPLACE FUNCTION mifracc.vf_revocar(p_card_id uuid, p_motivo text DEFAULT NULL)
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER
SET search_path = mifracc, auth AS $$
DECLARE v_n integer;
BEGIN
  ...
  UPDATE mifracc.card_requests
   ...
   WHERE id = p_card_id AND tipo = 'visita'
     AND colonia_id = mifracc.my_colonia_id() AND estado <> 'cancelada';
  GET DIAGNOSTICS v_n = ROW_COUNT;
...
GRANT EXECUTE ON FUNCTION mifracc.vf_listar() TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION mifracc.vf_editar(uuid, text) TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION mifracc.vf_revocar(uuid, text) TO authenticated, service_role;
