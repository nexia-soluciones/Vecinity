...
CREATE OR REPLACE FUNCTION mifracc.verificar_tarjeta_publico(p_clase text, p_id uuid)
...
BEGIN
  IF p_clase = 'r' THEN
    SELECT (p.is_active AND p.approval_status = 'aprobado'), c.nombre
      ...
      FROM mifracc.profiles p JOIN mifracc.colonias c ON c.id = p.colonia_id
     WHERE p.id = p_id;
  ELSIF p_clase = 'vf' THEN
    SELECT (r.estado IN ('impresa','entregada')), c.nombre
      INTO v_valida, v_colonia
      FROM mifracc.card_requests r JOIN mifracc.colonias c ON c.id = r.colonia_id
...
GRANT EXECUTE ON FUNCTION mifracc.verificar_tarjeta_publico(text, uuid) TO anon, authenticated, service_role;
