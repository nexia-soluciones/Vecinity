...
-- dueño externo). El vínculo dueño↔casa vive en mifracc.house_members. El
...
-- ------------------------------------------------------------
CREATE TYPE mifracc.house_relacion AS ENUM ('propietario');

CREATE TABLE mifracc.house_members (
  ...
  relacion    mifracc.house_relacion NOT NULL DEFAULT 'propietario',
  created_at  timestamptz NOT NULL DEFAULT now(),
  UNIQUE (house_id, profile_id, relacion)
);

CREATE INDEX idx_house_members_profile ON mifracc.house_members(profile_id);
CREATE INDEX idx_house_members_house   ON mifracc.house_members(house_id);

ALTER TABLE mifracc.house_members ENABLE ROW LEVEL SECURITY;

CREATE POLICY house_members_self_read ON mifracc.house_members FOR SELECT
  USING (profile_id = auth.uid());
CREATE POLICY house_members_admin ON mifracc.house_members FOR ALL
...
GRANT ALL ON mifracc.house_members TO anon, authenticated, service_role;

-- ------------------------------------------------------------
-- 2) Helper: casas cuyas FINANZAS puedo ver/operar
--    (donde vivo ∪ donde soy propietario). SOLO para superficies financieras.
-- ------------------------------------------------------------
CREATE OR REPLACE FUNCTION mifracc.my_finance_house_ids()
RETURNS SETOF uuid LANGUAGE sql SECURITY DEFINER STABLE
SET search_path = mifracc, auth AS $$
  SELECT house_id FROM mifracc.profiles
   WHERE id = auth.uid() AND house_id IS NOT NULL
  UNION
  SELECT house_id FROM mifracc.house_members
   WHERE profile_id = auth.uid()
$$;

GRANT EXECUTE ON FUNCTION mifracc.my_finance_house_ids() TO anon, authenticated, service_role;

-- ------------------------------------------------------------
-- 3) Invitación con relación: PROP-<casa> reutiliza mifracc.invitations
--    (relacion NULL = invitación de residente normal, como hasta hoy)
-- ------------------------------------------------------------
ALTER TABLE mifracc.invitations
  ADD COLUMN IF NOT EXISTS relacion mifracc.house_relacion;

-- RPC del comité: genera (o recupera, idempotente) el código PROP de una casa.
CREATE OR REPLACE FUNCTION mifracc.crear_invitacion_propietario(p_house_id uuid)
RETURNS jsonb
...
BEGIN
  ...
  SELECT token INTO v_token FROM mifracc.invitations
  ...
  INSERT INTO mifracc.invitations (colonia_id, house_id, role, relacion, token, invited_by, expires_at)
...
GRANT EXECUTE ON FUNCTION mifracc.crear_invitacion_propietario(uuid) TO authenticated, service_role;
...
CREATE POLICY transactions_read ON mifracc.transactions
  FOR SELECT USING (
    house_id IN (SELECT mifracc.my_finance_house_ids()) OR mifracc.is_admin()
  );
...
-- ------------------------------------------------------------
DROP FUNCTION IF EXISTS mifracc.registrar_abono(numeric, text, text, text);
...
GRANT EXECUTE ON FUNCTION mifracc.registrar_abono(numeric,text,text,text,uuid) TO authenticated, service_role;
      ...
      SELECT DISTINCT p.telegram_chat_id
       ...
       WHERE p.telegram_chat_id IS NOT NULL
         AND (p.house_id = NEW.id
              OR p.id IN (SELECT hm.profile_id FROM mifracc.house_members hm
