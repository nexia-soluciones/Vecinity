...
CREATE TABLE IF NOT EXISTS mifracc.comunicados (
  id                uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  colonia_id        uuid NOT NULL REFERENCES mifracc.colonias(id) ON DELETE CASCADE,
  house_id          uuid REFERENCES mifracc.houses(id) ON DELETE CASCADE,  -- NULL = público
...
CREATE INDEX IF NOT EXISTS idx_comunicados_colonia ON mifracc.comunicados(colonia_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_comunicados_house ON mifracc.comunicados(house_id) WHERE house_id IS NOT NULL;

ALTER TABLE mifracc.comunicados ENABLE ROW LEVEL SECURITY;

-- Lectura: el residente ve los públicos de su colonia + los dirigidos a su casa; el comité ve todos.
DROP POLICY IF EXISTS comunicados_read ON mifracc.comunicados;
CREATE POLICY comunicados_read ON mifracc.comunicados FOR SELECT USING (
  colonia_id = mifracc.my_colonia_id()
  AND (house_id IS NULL OR house_id = mifracc.my_house_id() OR mifracc.is_admin())
);

-- Escritura directa: solo admin (además hay RPC). Update para marcar leído va por RPC.
DROP POLICY IF EXISTS comunicados_admin ON mifracc.comunicados;
CREATE POLICY comunicados_admin ON mifracc.comunicados FOR ALL USING (
  colonia_id = mifracc.my_colonia_id() AND mifracc.is_admin()
) WITH CHECK (colonia_id = mifracc.my_colonia_id() AND mifracc.is_admin());

GRANT SELECT ON mifracc.comunicados TO authenticated;
GRANT ALL ON mifracc.comunicados TO service_role;
...
CREATE OR REPLACE FUNCTION mifracc.marcar_comunicado_leido(p_id uuid)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
DECLARE v_rows int;
BEGIN
  UPDATE mifracc.comunicados
     SET leido_at = coalesce(leido_at, now())
   WHERE id = p_id
     AND colonia_id = mifracc.my_colonia_id()
     AND (house_id = mifracc.my_house_id() OR house_id IS NULL);
  GET DIAGNOSTICS v_rows = ROW_COUNT;
  RETURN jsonb_build_object('ok', v_rows > 0);
END $$;

GRANT EXECUTE ON FUNCTION mifracc.crear_comunicado(uuid,text,text,text,boolean,text) TO authenticated;
GRANT EXECUTE ON FUNCTION mifracc.marcar_comunicado_leido(uuid) TO authenticated;
