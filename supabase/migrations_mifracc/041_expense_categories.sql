...
CREATE TABLE IF NOT EXISTS mifracc.expense_categories (
...
CREATE UNIQUE INDEX IF NOT EXISTS expense_categories_uq
  ON mifracc.expense_categories (colonia_id, lower(nombre));

ALTER TABLE mifracc.expense_categories ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS expense_categories_read  ON mifracc.expense_categories;
DROP POLICY IF EXISTS expense_categories_admin ON mifracc.expense_categories;
CREATE POLICY expense_categories_read ON mifracc.expense_categories FOR SELECT
  USING (colonia_id = mifracc.my_colonia_id());
CREATE POLICY expense_categories_admin ON mifracc.expense_categories FOR ALL
...
GRANT SELECT, INSERT, UPDATE, DELETE ON mifracc.expense_categories TO authenticated, service_role;

-- Semilla: 21 canónicas (orden = posición) + Insumos caseta, para cada colonia
INSERT INTO mifracc.expense_categories (colonia_id, nombre, orden)
...
CREATE OR REPLACE FUNCTION mifracc.crear_categoria(p_nombre text)
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER
SET search_path = mifracc, auth AS $$
DECLARE v_col uuid := mifracc.my_colonia_id(); v_id uuid; v_react boolean := false;
BEGIN
  ...
  SELECT id, NOT activa INTO v_id, v_react FROM mifracc.expense_categories
   WHERE colonia_id = v_col AND lower(nombre) = lower(btrim(p_nombre));
  IF v_id IS NOT NULL THEN
    UPDATE mifracc.expense_categories SET activa = true, nombre = btrim(p_nombre) WHERE id = v_id;
...
END $$;
GRANT EXECUTE ON FUNCTION mifracc.crear_categoria(text) TO authenticated, service_role;

-- --- RPC: renombrar (arrastra los gastos y el mapa aprendido a la vez) -------
CREATE OR REPLACE FUNCTION mifracc.renombrar_categoria(p_id uuid, p_nombre text)
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER
SET search_path = mifracc, auth AS $$
DECLARE v_col uuid := mifracc.my_colonia_id(); v_old text; v_new text := btrim(p_nombre);
BEGIN
  IF NOT mifracc.is_admin() THEN RAISE EXCEPTION 'Solo el comité.'; END IF;
  IF v_new = '' THEN RAISE EXCEPTION 'Escribe el nombre.'; END IF;

  SELECT nombre INTO v_old FROM mifracc.expense_categories
  ...
  IF lower(v_new) <> lower(v_old) AND EXISTS (
       SELECT 1 FROM mifracc.expense_categories
  ...
  UPDATE mifracc.expense_categories SET nombre = v_new WHERE id = p_id;
  -- arrastrar el histórico y el mapa aprendido para que todo quede consistente
  UPDATE mifracc.colonia_expenses SET categoria = v_new
   WHERE colonia_id = v_col AND categoria = v_old;
  UPDATE mifracc.expense_cat_map SET categoria = v_new
   WHERE colonia_id = v_col AND categoria = v_old;
  RETURN jsonb_build_object('ok', true);
END $$;
GRANT EXECUTE ON FUNCTION mifracc.renombrar_categoria(uuid,text) TO authenticated, service_role;

-- --- RPC: activar / desactivar (no borra: el histórico conserva su categoría) -
CREATE OR REPLACE FUNCTION mifracc.set_categoria_activa(p_id uuid, p_activa boolean)
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER
SET search_path = mifracc, auth AS $$
DECLARE v_col uuid := mifracc.my_colonia_id();
BEGIN
  IF NOT mifracc.is_admin() THEN RAISE EXCEPTION 'Solo el comité.'; END IF;
  UPDATE mifracc.expense_categories SET activa = coalesce(p_activa,true)
   WHERE id = p_id AND colonia_id = v_col;
  IF NOT FOUND THEN RAISE EXCEPTION 'Categoría no encontrada.'; END IF;
  RETURN jsonb_build_object('ok', true);
END $$;
GRANT EXECUTE ON FUNCTION mifracc.set_categoria_activa(uuid,boolean) TO authenticated, service_role;
