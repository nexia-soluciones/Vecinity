...
DROP FUNCTION IF EXISTS mifracc.cotizar_tarjeta(mifracc.card_type);
CREATE OR REPLACE FUNCTION mifracc.cotizar_tarjeta(
  p_tipo mifracc.card_type, p_personalizada boolean DEFAULT false)
...
DROP FUNCTION IF EXISTS mifracc.solicitar_tarjeta(mifracc.card_type, uuid, uuid, text, text);
CREATE OR REPLACE FUNCTION mifracc.solicitar_tarjeta(
  p_tipo mifracc.card_type, p_vehicle_id uuid, p_beneficiario uuid,
...
BEGIN
  ...
  v_cot := mifracc.cotizar_tarjeta(p_tipo, p_personalizada);
...
CREATE OR REPLACE FUNCTION mifracc.subir_comprobante_tarjeta(p_id uuid, p_url text)
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path = mifracc, public AS $$
DECLARE v_n int;
BEGIN
  ...
  UPDATE mifracc.card_requests
   ...
   WHERE id = p_id AND house_id = mifracc.my_house_id()
...
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path = mifracc, public AS $$
DECLARE r mifracc.card_requests%ROWTYPE;
BEGIN
  ...
  IF NOT FOUND OR r.colonia_id <> mifracc.my_colonia_id() THEN
...
UPDATE mifracc.card_requests cr
