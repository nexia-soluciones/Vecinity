...
BEGIN
  ...
  IF p_house_id IS NOT NULL
     AND NOT EXISTS (SELECT 1 FROM mifracc.houses WHERE id = p_house_id AND colonia_id = v_col) THEN
    RAISE EXCEPTION 'La casa no pertenece a tu colonia.';
  END IF;
  UPDATE mifracc.service_providers
...
CREATE OR REPLACE FUNCTION mifracc.baja_proveedor(p_id uuid)
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER
SET search_path = mifracc, auth AS $$
DECLARE v_n integer;
BEGIN
  IF NOT mifracc.is_guard() THEN RAISE EXCEPTION 'Solo el personal de vigilancia.'; END IF;
  UPDATE mifracc.service_providers SET activo = false
   WHERE id = p_id AND colonia_id = mifracc.my_colonia_id() AND activo;
  GET DIAGNOSTICS v_n = ROW_COUNT;
  IF v_n = 0 THEN RAISE EXCEPTION 'Proveedor inexistente, de otra colonia o ya dado de baja.'; END IF;
  RETURN jsonb_build_object('ok', true);
END $$;

GRANT EXECUTE ON FUNCTION mifracc.editar_proveedor(uuid, text, uuid, text) TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION mifracc.baja_proveedor(uuid) TO authenticated, service_role;

-- Tarjetas VF: el vigilante ya NO edita ni revoca (decisión 2026-07-16).
REVOKE EXECUTE ON FUNCTION mifracc.vf_editar(uuid, text) FROM authenticated;
REVOKE EXECUTE ON FUNCTION mifracc.vf_revocar(uuid, text) FROM authenticated;
