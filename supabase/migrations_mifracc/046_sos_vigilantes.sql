...
-- ------------------------------------------------------------
CREATE TABLE mifracc.vigilantes (
  id            uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  colonia_id    uuid NOT NULL REFERENCES mifracc.colonias(id) ON DELETE CASCADE,
  profile_id    uuid NOT NULL REFERENCES mifracc.profiles(id) ON DELETE CASCADE UNIQUE,
  ...
  resuelto_por  uuid REFERENCES mifracc.profiles(id) ON DELETE SET NULL
);
CREATE INDEX idx_vigilantes_colonia ON mifracc.vigilantes(colonia_id, estado);

ALTER TABLE mifracc.vigilantes ENABLE ROW LEVEL SECURITY;
CREATE POLICY vigilantes_self_read ON mifracc.vigilantes FOR SELECT
  USING (profile_id = auth.uid());
CREATE POLICY vigilantes_admin ON mifracc.vigilantes FOR ALL
...
GRANT ALL ON mifracc.vigilantes TO anon, authenticated, service_role;

-- Postularse (el propio vecino; requiere vivir en una casa)
CREATE OR REPLACE FUNCTION mifracc.postular_vigilante()
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER
SET search_path = mifracc, auth AS $$
DECLARE v_house uuid := mifracc.my_house_id(); v_col uuid := mifracc.my_colonia_id(); v_est text;
BEGIN
  IF v_house IS NULL THEN
    RAISE EXCEPTION 'Para ser vecino vigilante necesitas vivir en la colonia.';
  END IF;
  SELECT estado INTO v_est FROM mifracc.vigilantes WHERE profile_id = auth.uid();
  ...
  INSERT INTO mifracc.vigilantes (colonia_id, profile_id, estado)
...
CREATE OR REPLACE FUNCTION mifracc.resolver_vigilante(p_id uuid, p_accion text)
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER
SET search_path = mifracc, auth AS $$
DECLARE v_rows int;
BEGIN
  ...
  END IF;
  UPDATE mifracc.vigilantes
     SET estado = CASE p_accion WHEN 'aprobar' THEN 'aprobado' ELSE 'baja' END,
         resuelto_at = now(), resuelto_por = auth.uid()
   WHERE id = p_id AND colonia_id = mifracc.my_colonia_id();
  GET DIAGNOSTICS v_rows = ROW_COUNT;
  IF v_rows = 0 THEN RAISE EXCEPTION 'No existe esa postulación.'; END IF;
  RETURN jsonb_build_object('ok', true);
END $$;

GRANT EXECUTE ON FUNCTION mifracc.postular_vigilante() TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION mifracc.resolver_vigilante(uuid,text) TO authenticated, service_role;

-- ------------------------------------------------------------
-- 2) sos_events: registro de la ruta 911
-- ------------------------------------------------------------
ALTER TABLE mifracc.sos_events ADD COLUMN IF NOT EXISTS llamo_911 boolean NOT NULL DEFAULT false;

-- ------------------------------------------------------------
-- 3) disparar_sos: resuelve casa Y ZONA (fix capitán) + regresa datos 911
-- ------------------------------------------------------------
CREATE OR REPLACE FUNCTION mifracc.disparar_sos(
...
BEGIN
  ...
  SELECT h.zone_id, h.numero, h.street INTO v_zone, v_num, v_street
    FROM mifracc.houses h WHERE h.id = v_house;
  SELECT nombre INTO v_colonia FROM mifracc.colonias WHERE id = v_col;

  INSERT INTO mifracc.sos_events (colonia_id, profile_id, house_id, zone_id, mode, lat, lng)
  VALUES (v_col, auth.uid(), v_house, v_zone,
          CASE WHEN p_mode = 'silent' THEN 'silent'::mifracc.sos_mode ELSE 'loud'::mifracc.sos_mode END,
          p_lat, p_lng)
...
CREATE OR REPLACE FUNCTION mifracc.sos_marcar_911(p_id uuid)
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER
SET search_path = mifracc, auth AS $$
BEGIN
  UPDATE mifracc.sos_events SET llamo_911 = true
   WHERE id = p_id AND profile_id = auth.uid();
  RETURN jsonb_build_object('ok', true);
END $$;

GRANT EXECUTE ON FUNCTION mifracc.disparar_sos(double precision,double precision,text) TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION mifracc.sos_marcar_911(uuid) TO authenticated, service_role;

-- ------------------------------------------------------------
-- 4) tg_send_kb: Telegram con botones inline (pg_net, texto plano)
-- ------------------------------------------------------------
CREATE OR REPLACE FUNCTION mifracc.tg_send_kb(p_chat text, p_text text, p_kb jsonb)
...
-- ------------------------------------------------------------
CREATE OR REPLACE FUNCTION mifracc._sos_destinatarios(p_colonia uuid, p_zone uuid, p_excluir uuid)
...
SET search_path = mifracc AS $$
  ...
  WHERE p.colonia_id = p_colonia
    ...
    AND ( p.role IN ('comite','admin')
          OR p.id = (SELECT captain_id FROM mifracc.zones WHERE id = p_zone)
          OR EXISTS (SELECT 1 FROM mifracc.vigilantes v
...
BEGIN
  SELECT p.nombre INTO v_sender FROM mifracc.profiles p WHERE p.id = NEW.profile_id;
  SELECT h.numero, h.street INTO v_casa, v_calle FROM mifracc.houses h WHERE h.id = NEW.house_id;
  SELECT nombre INTO v_zona FROM mifracc.zones WHERE id = NEW.zone_id;
  SELECT nombre INTO v_colonia FROM mifracc.colonias WHERE id = NEW.colonia_id;
  ...
  v_msg := '🚨 SOS / Botón de pánico' || E'\n' ||
           ...
           CASE WHEN NEW.mode = 'silent' THEN E'\n⚠️ Modo silencioso: NO llamar al mifracc.' ELSE '' END ||
           v_maps || E'\n\n📞 Datos para el 911:' ||
  ...
  FOR r IN SELECT * FROM mifracc._sos_destinatarios(NEW.colonia_id, NEW.zone_id, NEW.profile_id)
  LOOP
    PERFORM mifracc.tg_send_kb(r.chat, v_msg,
...
-- ------------------------------------------------------------
CREATE OR REPLACE FUNCTION mifracc.bot_sos(p_token text, p_chat text)
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER
SET search_path = mifracc, auth AS $$
...
BEGIN
  ...
  RETURN mifracc.disparar_sos(NULL, NULL, 'loud');
END $$;

CREATE OR REPLACE FUNCTION mifracc.bot_sos_atender(p_token text, p_chat text, p_sos uuid)
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER
SET search_path = mifracc, auth AS $$
DECLARE
  p mifracc.profiles%ROWTYPE;
  s mifracc.sos_events%ROWTYPE;
  v_rows int; v_atendio text; v_solicitante_chat text; v_casa text; v_otros jsonb;
BEGIN
  p := mifracc._bot_perfil(p_token, p_chat);
  SELECT * INTO s FROM mifracc.sos_events WHERE id = p_sos;
  ...
  IF NOT (p.role IN ('comite','admin')
          OR EXISTS (SELECT 1 FROM mifracc.zones z WHERE z.id = s.zone_id AND z.captain_id = p.id)
          OR EXISTS (SELECT 1 FROM mifracc.vigilantes v WHERE v.profile_id = p.id AND v.estado = 'aprobado')) THEN
    RAISE EXCEPTION 'Solo comité, capitanes o vigilantes pueden atender un SOS.';
  END IF;

  -- primero gana (evita estampida de "yo voy")
  UPDATE mifracc.sos_events SET attended_by = p.id
   WHERE id = p_sos AND attended_by IS NULL;
  GET DIAGNOSTICS v_rows = ROW_COUNT;

  SELECT pr.nombre INTO v_atendio FROM mifracc.profiles pr
   WHERE pr.id = coalesce((SELECT attended_by FROM mifracc.sos_events WHERE id = p_sos), p.id);
  SELECT pr.telegram_chat_id INTO v_solicitante_chat FROM mifracc.profiles pr WHERE pr.id = s.profile_id;
  SELECT h.numero INTO v_casa FROM mifracc.houses h WHERE h.id = s.house_id;
  ...
  SELECT coalesce(jsonb_agg(d.chat), '[]'::jsonb) INTO v_otros
    FROM mifracc._sos_destinatarios(s.colonia_id, s.zone_id, s.profile_id) d
   WHERE d.chat <> p_chat;
...
CREATE OR REPLACE FUNCTION mifracc.bot_mi_vigilante(p_token text, p_chat text)
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER
SET search_path = mifracc, auth AS $$
DECLARE p mifracc.profiles%ROWTYPE; v_est text;
BEGIN
  p := mifracc._bot_perfil(p_token, p_chat);
  SELECT estado INTO v_est FROM mifracc.vigilantes WHERE profile_id = p.id;
  RETURN jsonb_build_object('ok', true, 'estado', v_est);
END $$;

REVOKE EXECUTE ON FUNCTION mifracc._sos_destinatarios(uuid,uuid,uuid) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION mifracc.bot_sos(text,text) TO anon, service_role;
GRANT EXECUTE ON FUNCTION mifracc.bot_sos_atender(text,text,uuid) TO anon, service_role;
GRANT EXECUTE ON FUNCTION mifracc.bot_mi_vigilante(text,text) TO anon, service_role;
