...
CREATE TABLE IF NOT EXISTS mifracc.rfid_service (
  id smallint PRIMARY KEY DEFAULT 1 CHECK (id = 1),
  orin_last_seen_at timestamptz,
  updated_at timestamptz NOT NULL DEFAULT now()
);
INSERT INTO mifracc.rfid_service (id) VALUES (1) ON CONFLICT DO NOTHING;
ALTER TABLE mifracc.rfid_service ENABLE ROW LEVEL SECURITY;
-- Sin policies: solo RPCs SECURITY DEFINER la tocan.

-- 3. Bitácora de cambios del panel --------------------------------------------
CREATE TABLE IF NOT EXISTS mifracc.rfid_panel_log (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  tipo text NOT NULL CHECK (tipo IN ('override','umbral')),
  house_id uuid REFERENCES mifracc.houses(id),
  colonia_id uuid REFERENCES mifracc.colonias(id),
  valor_anterior text,
...
ALTER TABLE mifracc.rfid_panel_log ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS rfid_panel_log_comite ON mifracc.rfid_panel_log;
CREATE POLICY rfid_panel_log_comite ON mifracc.rfid_panel_log
  FOR SELECT USING (mifracc.is_admin());

-- 4. RPC: fijar override de una casa (comité) ---------------------------------
CREATE OR REPLACE FUNCTION mifracc.rfid_set_override(
...
BEGIN
  ...
  SELECT rfid_override, numero INTO v_old, v_numero
    FROM mifracc.houses WHERE id = p_house_id FOR UPDATE;
  IF NOT FOUND THEN
  ...
  UPDATE mifracc.houses
   ...
   WHERE id = p_house_id;
  INSERT INTO mifracc.rfid_panel_log (tipo, house_id, valor_anterior, valor_nuevo, motivo, changed_by)
...
CREATE OR REPLACE FUNCTION mifracc.rfid_set_umbral(p_colonia_id uuid, p_umbral numeric)
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path = mifracc, public AS $$
...
BEGIN
  ...
  SELECT umbral_suspension_rfid, nombre INTO v_old, v_nombre
    FROM mifracc.colonias WHERE id = p_colonia_id FOR UPDATE;
  IF NOT FOUND THEN
    RAISE EXCEPTION 'Colonia no encontrada.';
  END IF;
  UPDATE mifracc.colonias SET umbral_suspension_rfid = p_umbral WHERE id = p_colonia_id;
  INSERT INTO mifracc.rfid_panel_log (tipo, colonia_id, valor_anterior, valor_nuevo, changed_by)
...
CREATE OR REPLACE FUNCTION mifracc.rfid_panel_data()
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path = mifracc, public AS $$
BEGIN
  IF NOT mifracc.is_admin() THEN
  ...
  RETURN jsonb_build_object(
    'orin_last_seen_at', (SELECT orin_last_seen_at FROM mifracc.rfid_service WHERE id = 1),
    'colonias', (
      ...
      SELECT COALESCE(jsonb_agg(to_jsonb(x) ORDER BY x.numero), '[]'::jsonb) FROM (
        SELECT h.id, h.numero, h.street, h.saldo,
               ...
               (SELECT COALESCE(jsonb_agg(jsonb_build_object(
                ...
                FROM mifracc.rfid_tags t WHERE t.house_id = h.id) AS tags
        FROM mifracc.houses h
        JOIN mifracc.colonias c ON c.id = h.colonia_id
        WHERE h.rfid_override <> 'auto'
           OR EXISTS (SELECT 1 FROM mifracc.rfid_tags t WHERE t.house_id = h.id)
      ) x
    ),
    'log', (
      SELECT COALESCE(jsonb_agg(to_jsonb(l) ORDER BY l.changed_at DESC), '[]'::jsonb) FROM (
        ...
        FROM mifracc.rfid_panel_log pl
        LEFT JOIN mifracc.houses h ON h.id = pl.house_id
        LEFT JOIN mifracc.colonias c ON c.id = pl.colonia_id
        LEFT JOIN mifracc.profiles pr ON pr.id = pl.changed_by
...
CREATE OR REPLACE FUNCTION mifracc.rfid_heartbeat(p_token text)
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path = mifracc, public AS $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM mifracc.bot_config WHERE token = p_token) THEN
    RAISE EXCEPTION 'token invalido';
  END IF;
  UPDATE mifracc.rfid_service SET orin_last_seen_at = now(), updated_at = now() WHERE id = 1;
...
DROP FUNCTION IF EXISTS mifracc.rfid_mark(text, uuid, text);
CREATE OR REPLACE FUNCTION mifracc.rfid_mark(
