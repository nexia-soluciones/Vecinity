...
-- ------------------------------------------------------------
CREATE TABLE mifracc.bot_config (
  id              boolean PRIMARY KEY DEFAULT true CHECK (id),
  token           text NOT NULL,
  casa_escalacion text NOT NULL DEFAULT '128',
  created_at      timestamptz NOT NULL DEFAULT now()
);
ALTER TABLE mifracc.bot_config ENABLE ROW LEVEL SECURITY;  -- sin policies: solo definer

INSERT INTO mifracc.bot_config (token)
VALUES (replace(gen_random_uuid()::text,'-','') || replace(gen_random_uuid()::text,'-',''));

-- ------------------------------------------------------------
-- 2) Sesiones conversacionales (flujos multi-paso)
-- ------------------------------------------------------------
CREATE TABLE mifracc.bot_sessions (
  chat_id    text PRIMARY KEY,
  step       text,
  data       jsonb NOT NULL DEFAULT '{}'::jsonb,
  updated_at timestamptz NOT NULL DEFAULT now()
);
ALTER TABLE mifracc.bot_sessions ENABLE ROW LEVEL SECURITY;  -- sin policies: solo definer

-- ------------------------------------------------------------
-- 3) Helpers internos
-- ------------------------------------------------------------
CREATE OR REPLACE FUNCTION mifracc._bot_check(p_token text)
...
CREATE OR REPLACE FUNCTION mifracc._bot_perfil(p_token text, p_chat text)
RETURNS mifracc.profiles LANGUAGE plpgsql SECURITY DEFINER
SET search_path = mifracc AS $$
DECLARE p mifracc.profiles%ROWTYPE;
BEGIN
  PERFORM mifracc._bot_check(p_token);
...
CREATE OR REPLACE FUNCTION mifracc._bot_como(p_profile uuid)
...
-- ------------------------------------------------------------
CREATE OR REPLACE FUNCTION mifracc.bot_perfil(p_token text, p_chat text)
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER
SET search_path = mifracc, auth AS $$
DECLARE p mifracc.profiles%ROWTYPE; v_casas jsonb;
BEGIN
  ...
  SELECT coalesce(jsonb_agg(jsonb_build_object(
   ...
   WHERE h.id IN (SELECT mifracc.my_finance_house_ids());
...
-- ------------------------------------------------------------
CREATE OR REPLACE FUNCTION mifracc.bot_areas(p_token text, p_chat text)
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER
SET search_path = mifracc, auth AS $$
...
BEGIN
  ...
  SELECT coalesce(jsonb_agg(jsonb_build_object(
    ...
    INTO v
    FROM mifracc.common_areas a
   WHERE a.colonia_id = p.colonia_id AND a.activa AND a.reservable;
  RETURN jsonb_build_object('ok', true, 'areas', v);
END $$;

CREATE OR REPLACE FUNCTION mifracc.bot_disponibilidad(p_token text, p_chat text, p_area uuid, p_fecha date)
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER
SET search_path = mifracc, auth AS $$
DECLARE p mifracc.profiles%ROWTYPE; v jsonb;
BEGIN
  ...
  SELECT coalesce(jsonb_agg(jsonb_build_object(
    ...
    INTO v FROM mifracc.disponibilidad_area(p_area, p_fecha) d;
  RETURN jsonb_build_object('ok', true, 'ocupadas', v);
END $$;

CREATE OR REPLACE FUNCTION mifracc.bot_crear_reserva(
...
BEGIN
  p := mifracc._bot_perfil(p_token, p_chat);
  PERFORM mifracc._bot_como(p.id);
  RETURN mifracc.crear_reserva(p_area, p_inicio, p_fin, p_personas);
END $$;

-- ------------------------------------------------------------
-- 6) Visitas: pase con token (URL la arma el bot)
-- ------------------------------------------------------------
CREATE OR REPLACE FUNCTION mifracc.bot_registrar_visita(
...
BEGIN
  p := mifracc._bot_perfil(p_token, p_chat);
  PERFORM mifracc._bot_como(p.id);
  RETURN mifracc.registrar_visita(p_nombre, p_fecha);
END $$;

-- ------------------------------------------------------------
-- 7) Abonos: registrar comprobante + OCR (mismos candados que la app)
-- ------------------------------------------------------------
CREATE OR REPLACE FUNCTION mifracc.bot_registrar_abono(
...
BEGIN
  p := mifracc._bot_perfil(p_token, p_chat);
  PERFORM mifracc._bot_como(p.id);
  RETURN mifracc.registrar_abono(p_monto, p_url, 'Abono vía Caty', p_hash, p_house);
END $$;

CREATE OR REPLACE FUNCTION mifracc.bot_set_abono_ocr(
...
BEGIN
  p := mifracc._bot_perfil(p_token, p_chat);
  PERFORM mifracc._bot_como(p.id);
  RETURN mifracc.set_abono_ocr(p_id, p_ocr, p_ref);
END $$;

-- ------------------------------------------------------------
-- 8) Saldo amable: casa(s) + últimos movimientos (contexto para Haiku)
-- ------------------------------------------------------------
CREATE OR REPLACE FUNCTION mifracc.bot_movimientos(p_token text, p_chat text, p_house uuid)
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER
SET search_path = mifracc, auth AS $$
DECLARE p mifracc.profiles%ROWTYPE; v jsonb; v_casa record;
BEGIN
  p := mifracc._bot_perfil(p_token, p_chat);
  PERFORM mifracc._bot_como(p.id);
  IF p_house NOT IN (SELECT mifracc.my_finance_house_ids()) THEN
    RAISE EXCEPTION 'Esa casa no es tuya.';
  END IF;
  SELECT numero, saldo, estatus INTO v_casa FROM mifracc.houses WHERE id = p_house;
  SELECT coalesce(jsonb_agg(jsonb_build_object(
    ...
    INTO v
    FROM (SELECT * FROM mifracc.transactions
...
-- ------------------------------------------------------------
CREATE INDEX IF NOT EXISTS idx_reglamento_fts ON mifracc.reglamento
...
BEGIN
  ...
  IF v = '[]'::jsonb THEN
    SELECT coalesce(jsonb_agg(jsonb_build_object(
      ...
      INTO v
      FROM (
        SELECT DISTINCT ON (rg.id) rg.* FROM mifracc.reglamento rg,
...
-- ------------------------------------------------------------
CREATE OR REPLACE FUNCTION mifracc.bot_escalar(p_token text, p_chat text, p_tema text, p_texto text)
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER
SET search_path = mifracc, auth AS $$
DECLARE p mifracc.profiles%ROWTYPE; v_casa text; v_chats jsonb; v_msg text;
BEGIN
  p := mifracc._bot_perfil(p_token, p_chat);
  SELECT numero INTO v_casa FROM mifracc.houses WHERE id = p.house_id;
  v_msg := '🙋 ' || coalesce(p.nombre,'Vecino') ||
           coalesce(' (casa '||v_casa||')', ' (sin casa: propietario)') ||
           ' necesita ayuda con ' || coalesce(p_tema,'un tema') || ':' || E'\n' ||
           coalesce(p_texto,'');
  INSERT INTO mifracc.notifications (colonia_id, house_id, tipo, mensaje, canal, estado_envio, enviado_at)
  ...
  FROM (
    SELECT pr.telegram_chat_id AS chat FROM mifracc.profiles pr
    ...
    UNION
    SELECT pr.telegram_chat_id FROM mifracc.profiles pr
      JOIN mifracc.houses h ON h.id = pr.house_id
      JOIN mifracc.bot_config bc ON bc.casa_escalacion = h.numero
...
-- ------------------------------------------------------------
CREATE OR REPLACE FUNCTION mifracc.bot_session_get(p_token text, p_chat text)
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER
SET search_path = mifracc AS $$
DECLARE s mifracc.bot_sessions%ROWTYPE;
BEGIN
  PERFORM mifracc._bot_check(p_token);
  SELECT * INTO s FROM mifracc.bot_sessions WHERE chat_id = p_chat;
  ...
  IF s.updated_at < now() - interval '2 hours' THEN
    DELETE FROM mifracc.bot_sessions WHERE chat_id = p_chat;
    RETURN jsonb_build_object('step', NULL, 'data', '{}'::jsonb);
  END IF;
  RETURN jsonb_build_object('step', s.step, 'data', s.data);
END $$;

CREATE OR REPLACE FUNCTION mifracc.bot_session_set(p_token text, p_chat text, p_step text, p_data jsonb DEFAULT '{}'::jsonb)
RETURNS void LANGUAGE plpgsql SECURITY DEFINER
SET search_path = mifracc AS $$
BEGIN
  PERFORM mifracc._bot_check(p_token);
  IF p_step IS NULL THEN
    DELETE FROM mifracc.bot_sessions WHERE chat_id = p_chat;
  ELSE
    INSERT INTO mifracc.bot_sessions (chat_id, step, data, updated_at)
...
-- ------------------------------------------------------------
REVOKE EXECUTE ON FUNCTION mifracc._bot_check(text) FROM PUBLIC, anon, authenticated;
REVOKE EXECUTE ON FUNCTION mifracc._bot_perfil(text,text) FROM PUBLIC, anon, authenticated;
REVOKE EXECUTE ON FUNCTION mifracc._bot_como(uuid) FROM PUBLIC, anon, authenticated;

GRANT EXECUTE ON FUNCTION mifracc.bot_perfil(text,text) TO anon, service_role;
GRANT EXECUTE ON FUNCTION mifracc.bot_areas(text,text) TO anon, service_role;
GRANT EXECUTE ON FUNCTION mifracc.bot_disponibilidad(text,text,uuid,date) TO anon, service_role;
GRANT EXECUTE ON FUNCTION mifracc.bot_crear_reserva(text,text,uuid,timestamptz,timestamptz,int) TO anon, service_role;
GRANT EXECUTE ON FUNCTION mifracc.bot_registrar_visita(text,text,text,timestamptz) TO anon, service_role;
GRANT EXECUTE ON FUNCTION mifracc.bot_registrar_abono(text,text,numeric,text,text,uuid) TO anon, service_role;
GRANT EXECUTE ON FUNCTION mifracc.bot_set_abono_ocr(text,text,uuid,jsonb,text) TO anon, service_role;
GRANT EXECUTE ON FUNCTION mifracc.bot_movimientos(text,text,uuid) TO anon, service_role;
GRANT EXECUTE ON FUNCTION mifracc.bot_reglamento_buscar(text,text,text) TO anon, service_role;
GRANT EXECUTE ON FUNCTION mifracc.bot_escalar(text,text,text,text) TO anon, service_role;
GRANT EXECUTE ON FUNCTION mifracc.bot_session_get(text,text) TO anon, service_role;
GRANT EXECUTE ON FUNCTION mifracc.bot_session_set(text,text,text,jsonb) TO anon, service_role;
