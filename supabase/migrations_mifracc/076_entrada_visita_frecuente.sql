...
-- puede registrar la entrada con un botón — se crea una fila en mifracc.visitors
-- (mismo historial y flujo de salida que cualquier visita) ligada a la tarjeta.

ALTER TABLE mifracc.visitors
  ADD COLUMN IF NOT EXISTS card_request_id uuid REFERENCES mifracc.card_requests(id);
CREATE INDEX IF NOT EXISTS visitors_card ON mifracc.visitors (card_request_id)
  WHERE card_request_id IS NOT NULL;

CREATE OR REPLACE FUNCTION mifracc.entrada_visita_frecuente(p_card_id uuid)
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER
SET search_path = mifracc, auth AS $$
DECLARE
  r mifracc.card_requests%ROWTYPE;
  v_adentro mifracc.visitors%ROWTYPE;
  v_casa text;
  v_id uuid;
BEGIN
  ...
  SELECT * INTO v_adentro FROM mifracc.visitors
...
GRANT EXECUTE ON FUNCTION mifracc.entrada_visita_frecuente(uuid) TO authenticated, service_role;
