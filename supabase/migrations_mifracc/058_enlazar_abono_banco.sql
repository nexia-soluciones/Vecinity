...
CREATE OR REPLACE FUNCTION mifracc._norm_ref_key(p text)
RETURNS text LANGUAGE sql IMMUTABLE
AS $$ SELECT upper(btrim(regexp_replace(coalesce(p,''), '\s+', ' ', 'g'))) $$;

CREATE OR REPLACE FUNCTION mifracc.enlazar_abono_banco(
  p_id         uuid,
  p_banco_hash text,
...
BEGIN
  ...
  UPDATE mifracc.transactions
   ...
   WHERE id = p_id;
  PERFORM mifracc.resolver_transaccion(p_id, true);  -- aprueba + saldo (trigger marca bank_movs)

  -- guarda el motivo también en la fila del banco (auditoría del enlace)
  UPDATE mifracc.bank_movs SET nota = coalesce('Enlace manual: ' || v_motivo, nota)
  ...
  v_ref := mifracc._norm_ref_key(b.concepto);
  IF v_ref <> '' THEN
...
END $$;
GRANT EXECUTE ON FUNCTION mifracc.enlazar_abono_banco(uuid, text, text) TO authenticated;
  ...
  FROM (
    ...
    LEFT JOIN LATERAL (
      SELECT * FROM (
        ...
        LEFT JOIN LATERAL (
          SELECT EXISTS (
            SELECT 1 FROM mifracc.bank_ref_map rm
             WHERE rm.colonia_id = v_col AND rm.house_id = t.house_id
               AND rm.ref_key = mifracc._norm_ref_key(b.concepto)
          ) AS si
        ) apr ON true
        LEFT JOIN LATERAL (
          SELECT
            (SELECT count(*) FROM mifracc.transactions t2
            ...
            (SELECT count(*) FROM mifracc.bank_movs b2
