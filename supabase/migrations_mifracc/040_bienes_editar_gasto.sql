...
ALTER TABLE mifracc.colonia_expenses ADD COLUMN IF NOT EXISTS es_bien boolean NOT NULL DEFAULT false;

-- Firma nueva (+p_es_bien) → tirar la anterior para no dejar sobrecarga ambigua
DROP FUNCTION IF EXISTS mifracc.clasificar_gasto(uuid,text,text,uuid,text,text,text);
...
END $$;
GRANT EXECUTE ON FUNCTION mifracc.clasificar_gasto(uuid,text,text,uuid,text,text,text,boolean) TO authenticated, service_role;
